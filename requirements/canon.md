...
```text
   서브모듈 고정       enode-design @ 0fd304b  (origin/main, 2026-09-02)
...
**`ADR-065` 는 「구현됨 `enode`」라고 적혀 있으나 `enode/main`(`1c290a6`) 에
`GET /v1/nodes` 가 없다.** 라우트는 15개 그대로다. 구현은 `enode` 저장소의
병합되지 않은 가지에 있으나 **이 팩은 그 가지를 참조하지 않는다** — v1 은
`1c290a6` 만 본다. U-관측API 는 `ADR-065` §2 의 응답 모양에서 새로 만든다.
