...
#### 3.4.3 고정 시나리오 제출
- 「새 작업」 → 화면 덮는 모달 → **서버측 allow-list 라우트**(Q2/A)로 **고정 시나리오만** 제출한다. 임의 계약은 거부. 제출자 이름 필수.
...
#### 3.4.4 제출자 이름
- run 에 `submitter` 표시 필드(Q3/A) — `GET /v1/runs` 목록에 노출 · 스텝에 주입. **작동 중 RUN 카드에 누가 낸 run 인지 보인다.**
