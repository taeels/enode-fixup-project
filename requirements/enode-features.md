...
**2026-09-08 전환 — 고정점이 대회 데모 장면으로 옮겨왔다** (`decisions.md` 8절).
`dhseo` LED 는 데모 **시나리오 ①** 로 접혔고, 완결성은 이제 데모 장면 완주로 잰다
(3.4 · `scene-gates.md` §5). 하드웨어 없는 참가자도 관람·참여하는 **공유 공개
데모**가 그 장면이다.
...
### 3.4 시연 — 대회 데모 (2026-09-08 전환)

가치의 고정점이 `dhseo` 장면에서 **대회 데모 장면**으로 옮겨왔다 (`decisions.md`
8절 · 1.5절). 하드웨어 없는 참가자도 관람·참여하는 **공유 공개 데모**다. 3.1~3.3 의
일곱은 이 데모가 딛는 기계로 남고, 아래 다섯이 데모 표면을 짓는다. 값은
`decisions.md` 8절이 닫았다.

#### 3.4.1 온보딩
- 첫 방문에 **강제**되는 카드 시퀀스(애니메이션) — 「왜 enode 인가: 보드를 효율적으로 운용한다」. 이후엔 안 뜨고 바로 데모 대시보드로.
- 대시보드 첫 방문엔 **가이드투어** — 웹캠 · 노드 현황판 · 현재 작업 목록 · 「새 작업」.
- 첫 방문 판정은 브라우저 localStorage. 화면 자리는 pen.dev(shonsin).

#### 3.4.2 게스트 공개 데모 대시보드
- 중앙 현황판(3.1.1)을 **공개 데모로 개조**. 토큰 입력 대신 **「Guest 로그인」** — 랜덤 2단어 이름을 준다.
- 메인 함대에 **3D 뷰(S6)**. 읽기 + 「새 작업」 버튼.
- 데모 Mediator 는 실 함대와 분리된 일회용 인스턴스 (`decisions.md` 8.3).
- **노드 표현이 강조하는 것 셋** (pen.dev 요구 · `시연시나리오변경.md`) — `capability` · `label` · `sandbox`. 노드 카드(C1)·3D 함대뷰(S6)가 이 셋을 두드러지게 보인다. `capability`·`label` 은 광고·`GET /v1/nodes` 에 이미 있고, `sandbox` 출처는 미정이다 (`decisions.md` 8.5).

#### 3.4.3 고정 시나리오 제출
- 「새 작업」 → 화면 덮는 모달 → **서버측 allow-list 라우트**(Q2/A)로 **고정 시나리오만** 제출한다. 임의 계약은 거부. 제출자는 Guest 로그인 이름(자동, 모달에 이름 칸 없음).
- 제출되면 **작업 그래프 화면(3D, S7)** 으로 전환. 그 화면이 run 목록을 안 가린다.
- 시나리오 둘 — ① rpi LED blink toggle(1초 heartbeat + persistent-on, 둘) ② mac 이 만든 「환영합니다」 wav 를 rpi 스피커로 재생.

#### 3.4.4 제출자 이름
- run 에 `submitter` 표시 필드(Q3/A · Guest 로그인 이름) — `GET /v1/runs` 목록에 노출 · 스텝에 주입. **작동 중 RUN 카드에 누가 낸 run 인지 보인다.**

#### 3.4.5 웹캠 플로팅 뷰
- 실물 보드를 고정 노트북이 송출 → **호스트형 브로드캐스트 임베드**(Q4/A). 대시보드 좌하단 **resizable 플로팅**, 레이아웃 불변. **Mediator 밖**이다.

**데모 시나리오 콘텐츠**:
- 노드 — rpi(windows `nacl1119@windows`, ssh 로 rpi 를 몬다) · mac(`shinson@mac`, higgsfield tts). 능력은 `decisions.md` 8.3.
- 시나리오 ②의 cross-node 음원은 봉인 Record blob 을 딛는다 (mac PUT blob → rpi needs → GET blob).

**수용 기준**: `scene-gates.md` §5 의 데모 장면 게이트 (CP8~CP11). 이 다섯이 데모
장면을 한 장면으로 만든다.
...
### 필수 — 시연 다섯 (2026-09-08 전환)

```text
   3.4.1 온보딩            첫 방문 카드 + 대시보드 가이드투어
   3.4.2 게스트 대시보드    Guest 로그인 · 랜덤 이름 · 3D 함대뷰(S6) · 새 작업
...
**2026-09-08 전환 — 장면 정본이 바뀐다** (`decisions.md` 8.2 Q1/A). 바로 아래
「앞의 넷이 `dhseo` 장면을 한 장면으로 만든다」는 **전환 전의 기록**이다. 지금
완결성을 재는 것은 **대회 데모 장면**이고, 옛 `CP4` 의 자리를 데모 완주 게이트
(`scene-gates.md` §5 의 `CP10`)가 진다. `dhseo` 의 LED 는 데모 시나리오 ① 로 접혔다.
일곱은 사라지지 않는다 — 데모가 딛는 기계다.
