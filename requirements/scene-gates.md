...
| | 조각 | 확인 (실동작) | 딛는 기능 | 일정 |
|---|---|---|---|---|
| CP0 | 기동이 안 깨졌다 | 기존 테스트 전부 초록. 커버리지 · 린트 · glyphscan 게이트 통과. 기존 라우트 15개 회귀. 경계 검사 테스트(`constraints.md` 의 구조 불변식) 초록 | 3.1.1 의 바닥 | 1일차 오전 |
| CP1 | 보인다 | curl 로 `GET /v1/runs` · `GET /v1/nodes` 가 실데이터 JSON. 임대(`lease.not_after`)와 `draining` 이 실려 있다 | 3.1.1 | 1일차 정오 |
| CP2 | 기다린다 | 노드 하나에 Run 둘을 던진다. 둘째가 `202` 와 `QUEUED`. 첫째가 끝나면 둘째가 `RUNNING` 으로 간다. 함대에 없는 능력은 여전히 `422` -> `FAILED` | 3.2.1 | 1일차 오후 |
| CP3 | 돌려받는다 | `at-boundary` 로 drain. 도는 단계가 끝난 경계에서 임대가 풀린다. 풀린 노드는 후보에서 빠져 있다. drain 을 풀면 대기 Run 이 집는다 | 3.2.2 | 2일차 오전 |
| CP4 | 한 장면 | 1절 ① ~ ⑦ 을 끝까지. 현황판과 제어판이 같은 사실을 보인다. LED 가 새 패턴이다 | 전부 | 2일차 정오 — 이후는 광내기 |
| CP5 | 감싼다 | `runctl mcp` 를 stdio 로 띄우고 `tools/list` 가 여덟을 낸다. `run.submit` 으로 낸 Run 이 현황판에 그대로 뜬다 — 같은 자료다. `fleet.list` 가 `GET /v1/nodes` 와 글자까지 같다 | 3.3.1 | 장면 밖 — CP1 뒤 아무 때나 |
| CP6 | 보인다 (하네스) | **두 국면이다.** ① 도는 것 — 계약을 던져 놓고 제어판을 열면 **단계가 끝나기 전에** 카드에 하네스 출력이 흐른다. 상한을 넘겨도 안 깨지고 오래된 줄부터 밀리며, **다음 단계가 첫 글자를 쓸 때** 갈린다. ② 지난 것 — 이 노드가 한 Run 목록이 뜨고, 하나를 누르면 결과(`verdict.checks`)와 봉인된 트랜스크립트가 보인다. 데몬 로그 카드는 그대로 있다 — 둘이 다른 물건임이 화면에서 보인다 | 3.1.3 | 장면 밖 — CP4 뒤 아무 때나 |

**CP4 가 2일차 정오에 있는 것이 이 팩의 핵심이다.** 맨 끝에 두면 실패가
맨 끝에 온다.

**CP6 도 그 장면 밖이다.** 1절의 `dhseo` 장면에 트랜스크립트가 안 나오므로
**CP4 의 조건이 아니고, CP6 이 빨개도 CP4 는 초록이다.** 딛는 것이 호스트 제어판(3.1.2)의
...
돌고, drain(3.2.2) · 호스트 제어판(3.1.2)과 겹치는 자리가 없어 병렬로 간다.
