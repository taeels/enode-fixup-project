...
| CP9 | 낸다 | 「새 작업」 모달이 **고정 시나리오만** 낸다(임의 계약은 서버측 allow-list 가 거부). 제출자 이름이 필수이고 `GET /v1/runs` 와 RUN 카드에 뜬다. 제출 뒤 작업 그래프(3D)로 전환되고 run 목록을 안 가린다 | 사람 | 3.4.3 · 3.4.4 | 3.1.1 · 3.4.2 |
...
```text
   CP8   온보딩   첫 방문 → 카드 시퀀스 강제.  넘기면 데모 대시보드
         ...
         S6       함대가 3D 뷰로 보인다
   CP9   모달     「새 작업」 → 화면 덮는 모달.  고정 시나리오 목록만 · 제출자 이름 칸
...
```text
   ...
   CP9   새 작업 -> 모달.  고정 시나리오 ① 선택 + 이름 입력 -> 제출
