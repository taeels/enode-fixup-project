...
| 트랜스크립트 | MVP 밖 (4절). S4 화면은 자리만 | `runner.go` 의 tee 는 「그다음」 |
...
| 도구 집합 | **여덟.** `ADR-022` §9.3 의 다섯 + 이번 회차가 만든 둘 + `run.get` (5.3절) | `prep/GOALS.md` 가 「대시보드 시각화에 쓰인 것과 **동일한 자료**가 MCP 로 나가야 한다」를 요구한다. 다섯만 내면 `GET /v1/nodes` · `GET /v1/runs` 가 MCP 로 안 나가서 현황판과 자료가 갈린다 |
...
```text
   트랜스크립트 구독    여전히 이월이다.  툴은 한 방에 답하는 물건이라 구독을
                       어떻게 표현할지가 별도 결정이다 (prep/GOALS.md)
