...
| 항목 | 값 | 근거 |
|---|---|---|
| 범위 | MVP 넷 — 중앙 현황판 · 대기열 · drain · 호스트 제어판. 그리고 `scene-gates.md` 의 장면 절단면. **5절이 2026-09-05 에 다섯째(Mediator MCP)를, 6절이 2026-09-06 에 여섯째(트랜스크립트)를 더했다 — 이 행은 그날의 기록이므로 고치지 않는다. 지금 세는 값은 `enode-features.md` 4절의 여섯이다** | 이틀 예산. 넷이 `dhseo` 장면 하나를 만든다 |
...
| 새 패키지의 커버리지 | 패키지별 하한 80% 를 새 패키지에도 그대로 건다. `internal/panel` · `internal/api/ui` 는 DB 없이 도는 테스트로 채운다 | 하한을 집행하는 자리는 `ci.yml:265` 의 `awk -v floor=80` 이고 그 값은 거기 박혀 있다 — 새 패키지는 프로파일에 나타나는 순간 자동으로 걸린다. `.coverage-contract.yml` 은 게이트 입력이 아니라 기준선 스냅숏이고, 지금 낡았다 (`measured_at_commit fa444f2` 가 이 저장소에 없는 객체이고, 실측이 5,979 -> 6,200 statements · 87.5% -> 87.4%). 이 유닛은 그 파일을 안 고치되, 유닛을 닫을 때 낡은 정도를 한 줄로 적는다 |
