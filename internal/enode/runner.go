...
// 그게 구멍이 나는 방식이다 ★. 타임아웃·임대 워치독·cwd 도 마찬가지다.
// 그래서 Harness 는 ★ 순수 함수 둘 ★ (Argv · Decode) 만 내놓고 실행은 안 한다.
//
//	           ┌──────────────────────────────────────┐
//	runner ────┤ 환경 화이트리스트 (R1)                │
//	 (공용)    │ 타임아웃 · cwd · stdin               │
...
//
//	inject   Credentials 가 돌려준 것 (R1). 환경 맨 위에 얹힌다.
//	emit     스트림 사건. nil 이면 버린다.
func runHarness(ctx context.Context, h Harness, bin string, p AgentParams,
	prompt string, io IOPaths, inject map[string]string, emit func(Event)) ([]byte, HarnessResult) {

	env := harnessEnv(h.Env(), map[string]string{"OUT": io.Out, "IN": io.In}, inject)

	cmd := exec.CommandContext(ctx, bin, h.Argv(p, io)...)
	cmd.Dir = io.Dir
