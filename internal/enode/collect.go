...
// 위치를 우리 이름에 매핑해야 한다. 방법이 셋이고 순서가 있다:
//
...
// onlyRegularInside 는 일반 파일 이면서 워크스페이스 안인 것만 남긴다.
//
...
//
// 계약은 노드 주인이 아닌 사람이 내고, 워크스페이스는 리뷰 대상 코드다.
// 둘 다 신뢰할 수 없으므로 실경로로 비교한다.
//
// 실경로로 검사하고 원래 경로로 여는 것은 여기서 안전하다 — collect 는
// 명령이 끝난 뒤에 돌아서 심링크를 바꿔칠 프로세스가 남아 있지 않다.
...
// 되돌릴 수 있다. 이 잠금이 확실히 덮는 것은 하네스의 파일 도구다.
// ADR-042 이후로는 Bash 가 열려 있으므로 이것은 방어가 아니라 표지에
