...
// 매칭이 422 로 거절한다.
func TestFirstSuccessfulAdvertNotifies(t *testing.T) {
	...
	defer mu.Unlock()
	if ready == 0 {
		t.Fatal("the advert succeeded but nothing was notified")
	}
	// 한 번만 — 광고는 주기마다 도는데 ready 는 「떴다」이지 「건강하다」가 아니다.
	if ready != 1 {
		t.Fatalf("notified %d times — want once (the advert ran %d times)",
			ready, calls())
	}
}

// 광고가 실패하면 안 알린다 — 안 뜬 것을 떴다고 하면 그 다음이 422 다.
func TestAFailedAdvertDoesNotNotify(t *testing.T) {
	...
	defer mu.Unlock()
	if ready != 0 {
		t.Fatalf("the advert failed yet notified %d times", ready)
	}
}
