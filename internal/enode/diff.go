...
// 아무것도 안 걷혔다. 이건 안전망이 아니라 ★ 빠진 정규 경로 ★ 다.
//
...
//
// 그리고 `Prepare` 의 sanitize 가 여기서 ★ 두 번째 값 ★ 을 한다 —
...
// (Mediator 의 putBlob 이 413 으로 같은 말을 한다). 대신 ★ 완전한 요약 ★ 인
...
func repoDiff(ctx context.Context, dir string, limit int64) ([]byte, error) {
	// 프로젝트마다 ★ 자기 임시 인덱스 ★ 를 쓴다 — 진짜 인덱스를 안 건드리는
	// 이유는 단일 git 쪽과 같다.
