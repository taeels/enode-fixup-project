...
func Test계약의_필드가_프롬프트까지_간다(t *testing.T) {
	...
	if step.In.Prompt != "PROMPT_MARKER" {
		t.Fatalf("★ in.prompt 가 안 도착했다 ★: %q", step.In.Prompt)
	}
	if len(step.In.From) != 1 || step.In.From[0] != "survey" {
		t.Fatalf("★ in.from 이 안 도착했다 ★: %v", step.In.From)
	}
	if len(step.Feedback) != 1 || step.Feedback[0] != "survey" {
		t.Fatalf("★ feedback 이 안 도착했다 ★: %v", step.Feedback)
	}

	// ★ 그리고 프롬프트에 실렸는가 ★
	p := buildPrompt(step.In.Prompt, "/o", step.Out, step.Schema,
		map[string]string{"survey": "SURVEY_BODY"}, step.Attempt, step.Expands,
		step.Roles, step.RoleAttrs, step.Owed, step.Standing, step.Goal, step.EnvelopeKey)
...
//
// 문법은 ★ 무엇을 지켜야 하는지 ★, 모양은 ★ 필드가 어떻게 생겼는지 ★ 다.
