...
//
//	  훅이 계약 밖을 시킨다          "증거.txt 를 만들어라"    → 모델이 거절했다
//	                                                            (계약 지시와 충돌하자
//	                                                             모델이 계약을 택했다)
//	  훅이 계약을 상기한다           "요구된 진단 이 아직 없다" → 모델이 따랐다
//	                                                            프롬프트가 "파일은 만들지
//	                                                            마라" 였는데도. 7턴 $0.16.
//
...
type StopInput struct {
	...
	CWD       string `json:"cwd"`
	// StopHookActive 는 두 번째 호출부터 true다. 안 보면 영원히 돈다.
...
type HookArgs struct {
	...
	// Plan 은 계획 산출물의 이름이다 — expands 단계에만 있다 (ADR-046).
	//
...
func RunStopHook(a HookArgs, in io.Reader, out io.Writer) error {
	...
	//
	// 예전에는 StopHookActive 하나로 한 번 물었으면 끝이었다. 그래서
	...
	//
	// 무한 루프는 같은 이유로 또 막는 것이지 다른 문제를 짚는 것이
	...
	// 다 냈다. 그런데 계획이면 모양까지 본다 (ADR-046).
	//
	// 계약은 이미 유효한 계획을 요구하고 있다 — 어긴 것을 짚는 것은
	// 새 지시가 아니라 상기다. 훅이 계약 밖을 시키면 모델이 거절한다는
	// 실측(위 표)과 어긋나지 않는다.
...
// fingerprint 는 되묻기 사유를 짧고 안정된 열쇠로 만든다.
//
...
const (
	...
	seenMissing  = "missing"
	seenPlan     = "plan"
	// hookBlockBudget 은 한 단계에서 되물을 수 있는 총 횟수다.
	//
