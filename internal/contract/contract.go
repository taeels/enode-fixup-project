...
// capability 어휘는 둘이다 (ADR-019 · ADR-022 §5.2).
//
...
const (
	// CapabilityAgentReason 은 일을 하는 능력이다 — 함대의 대부분.
	// 노드는 한 종류이고 능력은 속성의 존재로 표현된다.
	CapabilityAgentReason = "agent.reason"
	// CapabilityOrchestration 은 계약을 짓는 능력이다 (ADR-022 §5).
	//
...
type Contract struct {
	...
	// Ledger 는 이 Run 이 무엇을 발견할 수 있는가다 (ADR-023 §6).
	// 없으면 scope:"run" 과 같다 = 오늘 그대로.
	Ledger *Ledger `json:"ledger,omitempty"`
}

// UnmetName 은 목표에 못 닿았다고 말하는 자리다 (ADR-054).
//
// _cannot 과 다르다 (ADR-038):
//
//	_cannot   이 단계를 못 하겠다 — 단계가 실패하고 재시도가 남는다
...
//
// 이것 자체가 분기다 — 획득 결과는 산출물이 아니라 즉시 아는 값이라
...
// 하며, 전부 자기보다 뒤여야 한다(종료가 정적으로 보장된다). 안 간 쪽은
...
type Acquire struct {
	...
	// Acquired · Unavailable 은 목적지 단계다.
	// 실패가 중단이 아니라 값이 되는 자리이고, 그래서 획득 실패로
...
type Ask struct {
	...
	// Answerers 는 답할 수 있는 사람이다 — 선언은 스텝에, 집행은 서버에.
	...
	// Show 는 질문과 함께 보여줄 산출물이다 (ADR-032 §4 보강).
	//
...
type AskTimeout struct {
	...
	// Then 은 오늘 "fail" 뿐이다. "default"(기본 답으로 진행)와
...
type See struct {
	// Ledger 는 원장 목록을 이 단계에 심을 것인가다.
	//
	//	"" · "none"  안 심는다 (기본. 오늘 동작)
	//	"list"       그 시점 목록을 심는다 — 본문이 아니다
	//
...
// 오염시킨다" 는 전부 「유도」가 만든 결함이라 원인째 사라졌다.
type Ledger struct {
...
type Work struct {
	// ID 는 Work 의 키다. 안 적으면 (system, change_id) 에서 유도한다
...
// WorkID 는 동일성을 우리가 정의하지 않는다는 결정의 형태다 (ADR-023 §6.5.2).
//
// 우리는 그 시스템이 「하나의 변경」이라 부르는 것의 식별자를 받아 적는다.
// Gerrit 에서 그 단위는 change number이고 Change-Id 해시가 아니다 —
// 해시는 여러 브랜치의 change 에 걸치므로 키가 아니다. cherry-pick 과 rebase 에서
// change number 는 유지되고, abandon 후 새로 올리면 새 번호 = 다른 Work다.
//
...
// 와이어에서 속성은 as/capability/count 와 같은 층의 형제 키로 온다:
//
...
// 저장되지 않아 produced 가 불만족이 된다 (ADR-020). 즉 검사가 두 겹이다.
//
...
const (
	...
	// KindAsk 는 사람이 수행하는 단계다 (ADR-032).
	// 노드도 Mediator 도 아닌 네 번째 실행 주체 — 답이 오면 끝난다.
	KindAsk
	// KindAcquire 는 Mediator 가 수행하는 단계다 (ADR-022 §7.5 · ADR-024).
	//
...
type Step struct {
	...
	// Expands 는 이 단계가 steps 를 늘린다는 표시다 (ADR-022 §6.3 갈래 A).
	//
	// 계획을 짓는 것은 에이전트이고, 지어진 계약을 제출하는 것은 enode다
	...
	// Produces 는 이 계획이 반드시 지어야 할 단계 이름이다 (ADR-049).
	//
	// 왜 필요한가 — 목표를 계약이 표현할 수 없었다
	//
	// success_when 은 제출 시점에 있는 단계만 지목할 수 있다(ErrCondUnknownID).
	...
	// ADR-043 의 「고칠 것이 없다」는 목표가 이미 선 뒤의 판단이다.
	Produces []string `json:"produces,omitempty"`

	// Adopt 는 이 계획의 제안을 묻지 않고 채택한다는 선언이다 (ADR-061).
	//
	...
	// ⇒ 왜 사람의 승인이 없는지를 봉인만 보고 알 수 있다.
	Adopt string `json:"adopt,omitempty"`

	// Needs 는 이 단계가 기다리는 단계들이다 (ADR-023 §4).
	//
	// 오늘까지 의존은 목록에서의 위치였다 — steps[] 가 리스트이므로
	// 리스트가 전순서를 주고, 그래서 한 번에 하나만 돌았다. needs 는 그
	// 전순서를 선언된 간선으로 바꾼다. 표현이 늘지 않는다 — steps[] 는
	// 평평한 목록 그대로이고 관계만 는다.
	//
	...
	// 것은 의존이 아니라 반복이고 그건 repeat 의 자리다 (dispatch 와 같다).
	Needs []string `json:"needs,omitempty"`

	// Acquire 는 실행 중에 새 자원을 잡는다 (ADR-022 §7.5 · ADR-024).
	//
	// I5 를 안 깬다 — ADR-024 가 「요구 자원」을 한 획득 요청의 범위로
	...
	// Ask 는 사람에게 묻는 단계다 (ADR-032).
	//
	// needs 가 차면 Mediator 가 이 단계를 ASKED로 만들고, 노드는 손 뗀다 —
	...
	// 질문의 형태는 단계의 schema 그대로다 — 새 폼 언어를 만들지 않는다.
	// 다만 웹 폼 렌더를 위해 평면 부분집합으로 제한한다 (askForm).
	Ask *Ask `json:"ask,omitempty"`

	// Release 는 여기서 놓는 역할들이다 (ADR-022 §7.4).
	//
	...
	// 그 역할을 쓰는 모든 단계가 이 단계의 조상일 것을 요구한다(§4.3).
	Release []string `json:"release,omitempty"`

	// See 는 이 단계가 무엇을 볼지다 — 시야의 축소 축 (ADR-023 §6.4).
	//
	// 순수 축소다 — see 로는 원장 밖을 못 본다. 그것이 검증 조건이다.
	// 넓히는 것은 ledger.scope 가 하고, 이쪽은 줄이기만한다.
	// 없으면 목록을 안 심는다 = 오늘 그대로.
	See *See `json:"see,omitempty"`

	// Dispatch 는 분기다 — 식을 평가하지 않고 이름을 고른다 (ADR-022 §7.2).
	//
	// 기각한 것은 「결과가 X 면 A」 라는 표현식이지 분기 자체가 아니다.
...
// Branches 는 이 단계가 갈림길로 지목하는 단계들이다.
//
...
type Condition struct {
	...
	// Changed 는 이 단계 안에 실제로 바뀐 워크스페이스 경로다 (ADR-037).
	//
	...
	// FleetHas 는 함대에 그런 노드가 실제로 서 있는가다 (ADR-058).
	//
	// 다른 술어와 무엇이 다른가 — 대상이 이 Run 밖이다.
	//
	//	produced   에이전트가 만든 파일이 있나               ← 저작한다
	//	exit_code  에이전트가 짠 명령의 종료코드             ← 저작한다
	...
	// MinCount 는 그런 노드가 몇 이상이어야 하는가다. 없으면 1.
	MinCount int `json:"min_count,omitempty"`
}

// IsFleet 은 이 조건이 단계가 아니라 함대를 보는가다 (ADR-058).
...
var (
	...
	// 그래서 이 목록은 문서에 있는 것이고, 문법(Grammar)이 계획에게
	// 가르치는 것은 오늘 읽히는 것 뿐이다.
	inKeys = []string{"prompt", "from", "diff"}
)

// knownKeys 는 map 필드에 모르는 키가 있으면 어디에 적어야 하는지와 함께 거절한다.
...
func (c Contract) Validate() error {
	...
	// 뒤 단계가 그것을 uses 로 쓴다. 잡기 전에 못 쓴다는 것은 아래에서
	...
	//
	// 남는 것은 순서다. 계획이 지은 단계는 언제나 뒤에 붙으므로
	// (next.Steps = 기존 + 계획), 약속한 이름이 자신이거나 앞이면
	// 그것은 이행이 아니라 계약 저자의 착각이다.
	for _, st := range c.Steps {
	...
	// 검사는 dispatch 의 j <= i 와 같은 모양이고 같은 것을 지킨다 —
	...
	for i, st := range c.Steps {
		...
		for _, t := range d.To {
			...
			// 뒤로 가야 하는 것은 분기가 아니라 반복이고 그건 repeat 의 자리다.
			if j <= i {
				return fmt.Errorf("step %q: dispatch.to target %q comes before this step; "+
					"branches must point forward", st.ID, t)
			}
		}
		// 고른 뒤에 닿을 수 있는가 (ADR-060 §2) — DAG 검사는 종료를
		// 보장하지 도달을 보장하지 않는다. 목적지가 안 간 쪽의 뒷단계에
	...
	for _, cond := range c.SuccessWhen {
		...
		// Verify 가 got=-1 로 비교해 언제나 거짓이 되고, 계약 저자는
		...
		// 공허한 조건이 하나 늘 뿐이고, 그것을 Verify 가 못 가려낸다.
...
// HasCondition 은 같은 조건이 이미 목록에 있는가다 (ADR-060 §4).
//
// 계약 저자가 쓴 조건을 계획이 다시 제안하는 것은 흔하다. 그때 그냥 붙이면
// 같은 조건이 두 번 선다 — 오늘은 사본이 같아 판정이 안 바뀌지만,
// Verify 가 대조된 조건의 수를 「전부 건너뛰었나」의 하한으로 쓰므로
...
// sameStrings 는 순서까지 같은가다. 조건의 목록은 계약 저자가 쓴 순서를
...
// skipClosure 는 이 분기가 chosen 을 골랐을 때 죽는 단계들을 계약 위에서
// 미리 돈다 (ADR-060 §2). store 의 propagateSkips 와 같은 규칙이다 —
// needs 가 전부 죽었을 때만 죽고, 하나라도 살아 있으면 산다.
//
// chosen 자신도 대상에 넣는다 — applyDispatch 가 고른 것을 되살리지만,
// 되살린 뒤에도 전파는 그대로 돌기 때문이다. 되살리는 것은 분기의 판단이지
...
func skipClosure(c Contract, index map[string]int, to []string, chosen string) map[string]bool {
	...
	for {
		...
		for i, st := range c.Steps {
			...
			// 그대로 보면 실행 시의 그래프와 다른 그래프를 검사하게 된다.
