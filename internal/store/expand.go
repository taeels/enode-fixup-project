...
type plan struct {
	...
	// SuccessWhen 은 제안이다 (ADR-033) — 효력이 없다.
	//
	// 판정 기준의 저자는 기계(계획)일 수 있으나, 그것이 효력을 얻는 유일한
	// 길은 목표를 준 사람의 답이다 — 이 expands 단계를 adopts 로 지목한
...
// 결과가 나쁜 것이 아니라 계약이 요구한 것을 못 낸 것이다.
func (s *Store) applyExpands(ctx context.Context, tx pgx.Tx, runID string, seq int) error {
	...
	// 계약은 이미 되돌아가 있고, 지금 보고는 다시 지은 계획이다.
	...
	if len(p.Steps) == 0 {
		...
		// 공허하게 참이다 (verdict.go). 뒷단계는 needs 를 따라 전파된다.
		return skipAdopters(ctx, tx, runID, c.Steps, st.ID)
	}
	...
	// 바꾼 근거가 없다. 두 번째부터는 무엇을 보고 다시 짰나가 남아야
...
// causeOf 는 그 계획 단계가 무엇을 보고 지었나를 단계 기록 경로로 준다.
//
...
// byAnswer 는 사람의 답이 채택한 판이다 (ADR-033).
func byAnswer(id string) string { return "answer:" + id }

// byYolo 는 계약이 미리 맡긴 채택이다 (ADR-061 §3).
...
// unmetReported 는 그 단계가 목표 미달을 보고했는지다 (ADR-054).
//
