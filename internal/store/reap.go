...
func (s *Store) Reap(ctx context.Context, log *slog.Logger) (int, error) {
	...
	// 무한 대기가 계약 저자의 선언이다. 시스템이 3분으로 짐작하지 않는다.
...
//
// 종료 시 임대를 해제한다 — I2
// hasFleetCondition 은 함대를 읽을 이유가 있는가다 — 없으면 안 읽는다.
...
func (s *Store) SettleIfDone(ctx context.Context, runID string) (string, error) {
	...
	if broke > 0 {
		...
		//
		// 왜 이 시점인가 — 우리는 수렴 시스템이 아니라 기록 시스템이다
		// (ADR-005 · I4). 쿠버네티스처럼 조건이 맞을 때까지 기다리지 않는다.
		// Run 이 끝나는 그 순간의 함대가 판정의 대상이고 봉인의 대상이다.
		//
