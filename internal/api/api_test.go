...
// 진짜 Postgres 를 쓴다. I5 는 트랜잭션으로 얻는 것이라
// 가짜 저장소로는 검증되지 않는다.
func newServer(t *testing.T) *httptest.Server {
	t.Helper()
	url := os.Getenv("ENODE_TEST_DATABASE_URL")
	if url == "" {
		t.Skip("ENODE_TEST_DATABASE_URL is unset — see scripts/testdb.sh")
	}
...
func TestSubmitRejectsBadContract(t *testing.T) {
	...
	cases := map[string]string{
		"no run_id": contractJSON("", []map[string]any{req("b", nil)}, []map[string]any{runStep("s", "b")}),
		// ADR-019 — 어휘는 agent.reason 하나뿐이다
		"old capability": `{"run_id":"r1","requires":[{"as":"b","capability":"build.linux"}],
		                   "steps":[{"id":"s","uses":"b","run":["true"]}]}`,
		// ADR-019 결정 3 — 단계는 두 종류. 암묵을 안 남긴다
		"both agent and run": `{"run_id":"r2","requires":[{"as":"b","capability":"agent.reason"}],
...
func TestSubmitIdempotent(t *testing.T) {
	...
	if first["run_id"] != again["run_id"] || again["state"] != first["state"] {
		t.Fatalf("a different run appeared: %v vs %v", first, again)
	}
}
...
func TestConcurrentSubmitAllOrNothing(t *testing.T) {
	...
	if won != 1 {
		t.Fatalf("I1 violated: %d won, want exactly one", won)
	}
...
func TestDryRunDoesNotAllocate(t *testing.T) {
	...
	if code != 200 {
		t.Fatalf("dry-run code=%d, want 200", code)
	}
	// 점유하지 않았으므로 진짜 제출이 통과해야 한다
	if code, _ := do(t, srv, "POST", "/v1/runs", c, nil); code != 201 {
		t.Fatalf("the dry-run took a hold: code=%d", code)
	}
	...
	code, _ = do(t, srv, "POST", "/v1/runs/dry-run",
		contractJSON("dry2", []map[string]any{req("b", map[string]any{"harness": "claude"})},
			[]map[string]any{runStep("s", "b")}), nil)
	if code != 200 {
		t.Fatalf("the dry-run saw a hold: code=%d", code)
	}
	...
	code, _ = do(t, srv, "POST", "/v1/runs/dry-run",
		contractJSON("dry3", []map[string]any{req("b", map[string]any{"board": "SoC-Z"})},
			[]map[string]any{runStep("s", "b")}), nil)
	if code != 422 {
		t.Fatalf("the dry-run did not return 422: code=%d", code)
	}
}
...
func TestAdvertReplacesNotMerges(t *testing.T) {
	...
	if code, _ := do(t, srv, "POST", "/v1/runs",
		...
		t.Fatalf("the board must exist at first: %d", code)
	}
	// 보드가 빠졌다고 다시 광고한다
	...
	code, _ := do(t, srv, "POST", "/v1/runs",
		contractJSON("b2", []map[string]any{req("b", map[string]any{"board": "SoC-X"})},
			[]map[string]any{runStep("s", "b")}), nil)
	if code != 422 {
		t.Fatalf("advertisements were merged: code=%d, want 422", code)
	}
}
...
func newServerFast(t *testing.T, opts ...func(*config.Config)) (*httptest.Server, *store.Store) {
	...
	url := os.Getenv("ENODE_TEST_DATABASE_URL")
	if url == "" {
		t.Skip("ENODE_TEST_DATABASE_URL is unset")
	}
	ctx := context.Background()
	st, err := store.Open(ctx, url)
	if err != nil {
	...
	cfg := config.Default()
	cfg.Token = token
	cfg.Claim.LongPollSeconds = 0 // 204 immediately in tests
...
func TestHeartbeatCarriesLease(t *testing.T) {
	...
	if l, _ := body["leases"].([]any); len(l) != 0 {
		t.Fatalf("a lease exists before anything was claimed: %v", body)
	}
	...
	if len(leases) != 1 {
		t.Fatalf("the lease is missing from the heartbeat response: %v", body)
	}
	...
	leases, _ = body["leases"].([]any)
	if len(leases) != 1 {
		t.Fatalf("the lease disappeared on renewal: %v", body)
	}
	if second := leases[0].(map[string]any)["not_after"].(string); second <= first {
		t.Fatalf("not_after did not move: %s → %s", first, second)
	}
}
...
func TestClaimGoesToExactlyOneNode(t *testing.T) {
	...
	for _, c := range got {
		if c == 200 {
			...
			t.Fatalf("claim code=%d — want 200 or 204", c)
		}
	}
	if won != 1 {
		t.Fatalf("one step was handed out %d times", won)
	}
}
...
func TestClaimRespectsOrder(t *testing.T) {
	...
	if code != 200 || first["name"] != "s1" {
		t.Fatalf("the first step did not come: %d %v", code, first)
	}
	// 1 단계가 끝나기 전에는 2 단계가 안 나온다
	if code, _ := do(t, srv, "POST", "/v1/nodes/n1/claim", "", nil); code != 204 {
		t.Fatalf("handed out the next step out of order: %d", code)
	}
	// 보고하면 나온다
	...
	if code != 200 || second["name"] != "s2" {
		t.Fatalf("the next step did not come: %d %v", code, second)
	}
}
...
func TestLeasesReleasedOnCompletion(t *testing.T) {
	...
	_, body := do(t, srv, "POST", "/v1/nodes", adv, nil)
	if l, _ := body["leases"].([]any); len(l) != 0 {
		t.Fatalf("I2 violated: a lease survived termination: %v", l)
	}
	// 자원이 다시 쓸 수 있어야 한다
	if code, _ := do(t, srv, "POST", "/v1/runs", oneStepRun("next", "n1"), nil); code != 201 {
		t.Fatalf("the resource was not released: %d", code)
	}
}
...
func TestReapReleasesExpiredLease(t *testing.T) {
	...
	if n != 1 {
		t.Fatalf("reaped runs=%d, want 1", n)
	}
...
func TestVerdictO4(t *testing.T) {
	...
	if run["state"] != "SUCCEEDED" {
		t.Fatalf("O4 failed: state=%v — a result value leaked into the pass criteria", run["state"])
	}
	if run["verdict"] == nil {
...
func TestNonZeroExitIsStillCompletion(t *testing.T) {
	...
	if run["state"] != "FAILED" {
		t.Fatalf("passed with exit 2: %v", run["state"])
	}
	...
	if run["state"] != "SUCCEEDED" {
		t.Fatalf("I3 violated: judged on something the contract never asked: %v", run["state"])
	}
}
...
func TestStepThatCouldNotRun(t *testing.T) {
	...
	do(t, srv, "POST", "/v1/runs/broke/steps/1/result",
		`{"node":"n1","error":"aborted: lease expired"}`, nil)
	...
	if code, _ := do(t, srv, "POST", "/v1/nodes/n1/claim", "", nil); code != 204 {
		t.Fatalf("the next step of a failed run was handed out: %d", code)
	}
}
...
func TestCancel(t *testing.T) {
	...
	if code, _ := do(t, srv, "POST", "/v1/runs/cx/cancel", "", nil); code != 200 {
		t.Fatalf("the second cancel returned %d", code)
	}
	...
	if l, _ := hb["leases"].([]any); len(l) != 0 {
		t.Fatalf("a lease survived the cancel: %v", l)
	}

	// 취소된 Run 의 단계는 배분되지 않는다
	if code, _ := do(t, srv, "POST", "/v1/nodes/n1/claim", "", nil); code != 204 {
		t.Fatalf("a step of a cancelled run was handed out: %d", code)
	}
	// 자원을 다시 쓸 수 있다
	if code, _ := do(t, srv, "POST", "/v1/runs", oneStepRun("after-cx", "n1"), nil); code != 201 {
		t.Fatalf("the resource was not released after the cancel: %d", code)
	}
	...
	if len(checks) == 0 || checks[0].(map[string]any)["what"] != "cancelled" {
		t.Fatalf("the cancel reason was not recorded: %v", run["verdict"])
	}
}
...
func TestJudgmentKeywordRejectedAtSubmit(t *testing.T) {
	...
	if code != 400 {
		t.Fatalf("a verdict keyword passed: code=%d", code)
	}
	if e, _ := body["error"].(map[string]any); e == nil ||
		!strings.Contains(e["reason"].(string), "minimum") {
		t.Fatalf("the offending keyword is not named: %v", body)
	}
}
...
func TestBlobSchemaViolationIsNotStored(t *testing.T) {
	...
	if e, _ := body["error"].(map[string]any); e == nil ||
		...
		t.Fatalf("the violation detail is too thin: %v", body) // it must feed back
	}
	// 저장되지 않았다
	if code, _ := do(t, srv, "GET", "/v1/runs/sv/blob/hypothesis", "", nil); code != 404 {
		t.Fatalf("a violating artifact was stored: code=%d", code)
	}
	// 정직한 답은 통과한다 — 부재가 아니라 값으로 "못 하겠다" 를 말한다
	if code, _ := do(t, srv, "PUT", "/v1/runs/sv/steps/1/blob/hypothesis",
		`{"status":"none","reason":"the patch only changes comments"}`, nil); code != 204 {
		t.Fatalf("an honest answer was rejected: %d", code)
	}
}
...
func TestBlobRefusedAfterTerminal(t *testing.T) {
	...
	if code, _ := do(t, srv, "PUT", "/v1/runs/term/steps/1/blob/x", "late", nil); code != 410 {
		t.Fatalf("written after sealing: code=%d", code)
	}
}
...
func TestRetryLoopIsDrivenByMediator(t *testing.T) {
	...
	if code != 200 || body["rolled_back"] != true {
		t.Fatalf("did not feed back and retry: %d %v", code, body)
	}

	// agent 단계가 다시 나와야 하고 회차가 올라 있어야 한다
	code, step := do(t, srv, "POST", "/v1/nodes/n1/claim", "", nil)
	if code != 200 || step["name"] != "write_test" {
		t.Fatalf("the retry step did not come: %d %v", code, step)
	}
	if a, _ := step["attempt"].(float64); a != 1 {
		t.Fatalf("attempt=%v, want 1 — nothing justifies putting the feedback in the prompt", step["attempt"])
	}
	if fb, _ := step["feedback"].([]any); len(fb) != 1 {
		t.Fatalf("the feedback names are missing: %v", step["feedback"])
	}
	...
	if run["state"] != "SUCCEEDED" {
		t.Fatalf("state=%v, want SUCCEEDED · %v", run["state"], run["verdict"])
	}
}
...
func TestRetryExhaustionFailsViaWithinAttempts(t *testing.T) {
	...
	if !found {
		t.Fatalf("exhaustion was not caught in the verdict: %v", v)
	}
}
...
func TestCapabilities(t *testing.T) {
	...
	if len(caps) != 1 {
		t.Fatalf("after ADR-019 there is one keyword: %v", body)
	}
	...
	if n, _ := c["nodes"].(float64); n != 2 {
		t.Fatalf("nodes=%v, want 2", c["nodes"])
	}
	attrs, _ := c["attrs"].(map[string]any)
	for _, k := range []string{"harness", "repo", "arch", "board", "tag"} {
		if _, ok := attrs[k]; !ok {
			t.Fatalf("attribute %q is not visible — a contract author cannot read the vocabulary: %v", k, attrs)
		}
	}
}
...
func TestCapabilitiesDoesNotLeakAvailability(t *testing.T) {
	...
	adv := advert("n1", "a", map[string]string{"role": "x"})
	do(t, srv, "POST", "/v1/nodes", adv, nil)
	do(t, srv, "POST", "/v1/runs", oneStepRun("busy", "n1"), nil) // takes a hold

	_, body := do(t, srv, "GET", "/v1/capabilities", "", nil)
	caps, _ := body["capabilities"].([]any)
	c := caps[0].(map[string]any)
	if n, _ := c["nodes"].(float64); n != 1 {
		t.Fatalf("hold state leaked into the vocabulary: nodes=%v, want the total", c["nodes"])
	}
	if _, leaked := c["free"]; leaked {
		t.Fatal("exposed free capacity — that invites a check-then-act race")
	}
}
...
func TestClaim_CarriesTheRequester(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("n1", "a", map[string]string{"role": "x"}), nil)
	...
	if got, _ := body["requester"].(string); got != "taeels@gmail.com" {
		t.Fatalf("the requester is missing: %q (want runs.principal)", got)
	}
}
...
func TestDispatch_PicksANameAndRunsOnlyThatSide(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("n1", "a", map[string]string{"role": "x"}), nil)
	...
	if code != 200 || next["name"] != "quick" {
		t.Fatalf("the chosen branch did not come: %d %v", code, next)
	}
}
...
func TestDispatch_AnUnchosenNameFailsThatStep(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("n1", "a", map[string]string{"role": "x"}), nil)
	...
	triage := map[string]any{
		...
		// 라우터까지 온다. 두 겹 중 두 번째 겹만 시험하는 것이다.
		"dispatch": map[string]any{"from": "route.next", "to": []string{"full", "quick"}},
	}
...
func TestNeeds_UnwrittenMeansTodaysOrder(t *testing.T) {
	...
	for i, want := range []string{"one", "two", "three"} {
		...
		if code != 200 || c["name"] != want {
			t.Fatalf("step %d came back as %v — want %q", i+1, c["name"], want)
		}
...
//
// 오늘(seq 게이트)이면 두 번째 노드는 앞 단계가 안 끝났다는 이유로 204 를
// 받는다. needs 가 열리면 서로 안 가리키므로 둘 다 집힌다.
func TestNeeds_TwoIndependentStepsAreClaimedTogether(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code1 != 200 || c1["name"] != "left" {
		t.Fatalf("p1 could not claim left: %d %v", code1, c1)
	}
	if code2 != 200 || c2["name"] != "right" {
		t.Fatalf("the second was not claimed concurrently: %d %v — "+
			"the total order is still there", code2, c2)
	}
}

// join 은 needs 가 전부 끝난 뒤에만 집힌다 — 폭을 열어도 합류는 기다린다.
func TestNeeds_AJoinIsClaimedOnlyAfterEverything(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	do(t, srv, "POST", "/v1/runs/joi/steps/1/result", `{"node":"j1","exit_code":0}`, nil) // only left finishes
	if code, c := do(t, srv, "POST", "/v1/nodes/j1/claim", "", nil); code == 200 {
		t.Fatalf("the join was claimed with only one side done: %v", c)
	}
	do(t, srv, "POST", "/v1/runs/joi/steps/2/result", `{"node":"j2","exit_code":0}`, nil) // right finishes too
	if code, c := do(t, srv, "POST", "/v1/nodes/j1/claim", "", nil); code != 200 || c["name"] != "join" {
		t.Fatalf("everything finished yet the join was not claimed: %d %v", code, c)
	}
}
...
func TestSkipped_PropagatesAlongTheEdges(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code != 200 || next["name"] != "quick" {
		t.Fatalf("a step behind an untaken branch survived: %d %v — "+
			"propagation did not follow the edges", code, next)
	}
}
...
// 회차마다 dispatch 가 서고, 조기종료 출구가 그 분기들의 공통 목적지가 된다.
...
func TestDispatch_ADestinationSharedByTwoBranchesComesBack(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("s1", "a", map[string]string{"role": "x"}), nil)
	...
	// final 은 SKIPPED 가 되고, 그것은 도달 가능성과 무관한 분기의 판단이다.
	...
	if code, _ := do(t, srv, "POST", "/v1/runs/dsh/steps/1/result",
		...
		t.Fatalf("verify_1 report failed: %d", code)
	}
	if code, c := do(t, srv, "POST", "/v1/nodes/s1/claim", "", nil); code != 200 || c["name"] != "fix_1" {
		t.Fatalf("fix_1 did not come: %d %v", code, c)
	}
	...
	if code, c := do(t, srv, "POST", "/v1/nodes/s1/claim", "", nil); code != 200 || c["name"] != "verify_2" {
		t.Fatalf("verify_2 did not come: %d %v", code, c)
	}
	...
	if code, _ := do(t, srv, "POST", "/v1/runs/dsh/steps/3/result",
		...
		t.Fatalf("verify_2 report failed: %d", code)
	}

	// 여기가 결함이 드러나는 자리다
	code, next := do(t, srv, "POST", "/v1/nodes/s1/claim", "", nil)
	if code != 200 || next["name"] != "final" {
		t.Fatalf("the chosen destination did not come back: %d %v — "+
			"final went SKIPPED because verify_1 did not pick it, and "+
			"verify_2 picking it did not undo that", code, next)
	}
}
...
// 문법에 loop 을 넣은 뒤 계획이 3 단계·dispatch 0 개로 지었다.
//
...
func TestLoop_APlannedRetryReachesTheGoal(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	fix := map[string]any{
		...
		"in":  map[string]any{"from": []string{"qemu_log"}, "prompt": "fix it"},
	...
	pass := func(name string, produced ...string) {
		...
		if code != 200 {
			t.Fatalf("could not claim %s: %d", name, code)
		}
		if c["name"] != name {
			t.Fatalf("want %s, got %v", name, c["name"])
		}
		...
		if code, _ := do(t, srv, "POST", "/v1/runs/lp2/steps/"+seq+"/result",
			...
			t.Fatalf("%s report failed: %d", name, code)
		}
	}
	...
	if code != 200 || c["name"] != "diagnose_fix" {
		t.Fatalf("the loop did not turn: %d %v — until is unmet yet "+
			"it did not go back to back_to", code, c)
	}
	...
	if v["state"] != "SUCCEEDED" {
		t.Fatalf("the goal was met, yet %v", v["state"])
	}
	raw, _ := json.Marshal(v["verdict"])
	if strings.Contains(string(raw), `"skipped"`) {
		t.Fatalf("the goal step was judged as skipped: %s — "+
			"this is how third-run-1 got sealed as SUCCEEDED", raw)
	}
	if !strings.Contains(string(raw), "verify_result") {
		t.Fatalf("the verdict never looked at the goal artifact: %s", raw)
	}
}
...
func TestExpands_APlanGrowsTheSteps(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, c := do(t, srv, "POST", "/v1/nodes/e1/claim", "", nil); code != 200 || c["name"] != "plan" {
		t.Fatalf("plan did not come: %d %v", code, c)
	}
	...
	if code, _ := do(t, srv, "PUT", "/v1/runs/exp/steps/1/blob/plan",
		...
		t.Fatalf("storing the plan failed: %d", code)
	}
	...
	if code != 200 || next["name"] != "built" {
		t.Fatalf("the built step was not claimed: %d %v", code, next)
	}
	// 이름만 오면 안 된다 — 실행 정의가 함께 와야 노드가 돌린다.
	// 이름은 steps 행에서 오고 정의는 계약에서 오는데, 제출 전문만 보면
	// 계획이 지은 단계의 정의가 비어서 온다 (실측에서 밟았다:
	// "명령 단계인데 run 이 비었다").
	raw, _ := json.Marshal(next["run"])
	if !strings.Contains(string(raw), "true") {
		t.Fatalf("the built step carries no run definition: run=%s — "+
			"claim does not read the grown contract", raw)
	}
}
...
func TestExpands_AnUnknownRoleFailsThatStep(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	do(t, srv, "PUT", "/v1/runs/exp2/steps/1/blob/plan",
		`{"steps":[{"id":"built","uses":"norole","run":["true"],"out":["built"]}]}`, nil)
	...
	if code, c := do(t, srv, "POST", "/v1/nodes/e2/claim", "", nil); code == 200 {
		t.Fatalf("an invalid plan reached the fleet: %v", c)
	}
}
...
func TestExpands_APlanCannotWriteTheCriteria(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, c := do(t, srv, "POST", "/v1/nodes/e3/claim", "", nil); code == 200 {
		t.Fatalf("criteria written by the plan got in: %v", c)
	}
}

// 봉인에 v2 가 남는다 (ADR-005 성질 1·4)
//
// v1 은 언제나 제출 전문이고 v2 는 계획이 지은 판이다. 앞 판을 고치지 않고
// 붙이므로, 봉인된 묶음만 열어서 「무엇을 요청했고 무엇이 지어졌나」를 안다.
func TestExpands_TheSealKeepsTheContractColumn(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if len(m.Contract) != 2 {
		t.Fatalf("%d versions — want two: v1 (submitted) and v2 (plan)", len(m.Contract))
	}
	if m.Contract[0].By != "requester" || len(m.Contract[0].Steps) != 1 {
		t.Fatalf("v1 is not the full submission: by=%q steps=%d — property 4 breaks",
			m.Contract[0].By, len(m.Contract[0].Steps))
	}
	if m.Contract[1].By != "step:plan" || len(m.Contract[1].Steps) != 2 {
		t.Fatalf("v2 is wrong: by=%q steps=%d", m.Contract[1].By, len(m.Contract[1].Steps))
	}
	if m.Contract[1].Evidence == "" {
		t.Fatal("what it was built from was not recorded — evidence is empty")
	}
}
...
// ⇒ 진행을 읽을 경로가 선택이 아니라 필수가 된다.

func TestObserve_ParallelBranchesAreEachVisible(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("o1", "left-machine", map[string]string{"role": "x"}), nil)
	do(t, srv, "POST", "/v1/nodes", advert("o2", "right-machine", map[string]string{"role": "y"}), nil)
	...
	if code, _ := do(t, srv, "GET", "/v1/runs/obs/record", "", nil); code != 409 {
		t.Fatalf("an unsealed record is not 409: %d — I4 wobbles", code)
	}
	code, v := do(t, srv, "GET", "/v1/runs/obs", "", nil)
	if code != 200 {
		t.Fatalf("observe failed: %d", code)
	}
	...
	if err := json.Unmarshal(raw, &steps); err != nil || len(steps) != 3 {
		t.Fatalf("no steps are visible: %v (%d)", string(raw), len(steps))
	}
	// 두 가지가 동시에 돌고 서로 다른 기계에 있다 — Case D 를 실행 중에 읽는다.
	if steps[0].State != "CLAIMED" || steps[1].State != "CLAIMED" {
		t.Fatalf("concurrency is not visible: %q %q", steps[0].State, steps[1].State)
	}
	if steps[0].Node == steps[1].Node || steps[0].Node == "" {
		t.Fatalf("which machine runs it is not visible: %q %q", steps[0].Node, steps[1].Node)
	}
	// 빈 needs 도 내보낸다 — [] 는 "안 기다린다" 는 뜻이고 가지의 시작점이다.
	if steps[0].Needs == nil || len(steps[0].Needs) != 0 {
		t.Fatalf("empty needs did not come: %v — the reader has to guess the default", steps[0].Needs)
	}
	if len(steps[2].Needs) != 2 {
		t.Fatalf("the join edges are not visible: %v", steps[2].Needs)
	}
	if steps[2].State != "PENDING" {
		t.Fatalf("an unclaimed step is %q", steps[2].State)
	}
}

// 건너뛴 가지가 관측에서도 보인다 — 결과가 없는 것과 다르다는 것이
// 실행 중에도 읽혀야 한다 (그러지 않으면 크래시와 구분이 안 된다).
func TestObserve_ASkippedBranchIsVisible(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if got["full"] != "SKIPPED" {
		t.Fatalf("the untaken branch does not read as SKIPPED: %q", got["full"])
	}
	if got["quick"] != "PENDING" {
		t.Fatalf("the chosen branch is %q", got["quick"])
	}
}

// ═══ 원장 — 시야를 순서에서 유도하지 않는다 (ADR-023 §6) ═════════════
//
// 조상인지 형제인지를 묻지 않고 그 시점 원장에 있는가만 본다.
...
func TestLedger_GivesTheListNotTheBody(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("L1", "a", map[string]string{"role": "x"}), nil)
	...
	do(t, srv, "PUT", "/v1/runs/led1/steps/1/blob/note", `{"line":"this line is the body"}`, nil)
	do(t, srv, "POST", "/v1/runs/led1/steps/1/result", `{"node":"L1","produced":["note"]}`, nil)

	code, v := do(t, srv, "GET", "/v1/runs/led1/ledger", "", nil)
	if code != 200 {
		t.Fatalf("ledger lookup failed: %d", code)
	}
	raw, _ := json.Marshal(v["entries"])
	if strings.Contains(string(raw), "this line is the body") {
		t.Fatalf("the body got into the ledger: %s — want a list", raw)
	}
	var es []struct {
	...
	if len(es) != 1 || es[0].Name != "note" {
		t.Fatalf("the ledger is %v", string(raw))
	}
	if es[0].By != "step:note" {
		t.Fatalf("who produced it is not visible: %q", es[0].By)
	}
	if es[0].Bytes == 0 {
		t.Fatal("the size is not visible — nothing to decide whether to fetch the body")
	}
	if es[0].RunID != "" {
		t.Fatalf("run_id was attached to an entry from the same run: %q — every line repeats one value", es[0].RunID)
	}
}
...
func TestLedger_WhatAnEarlierPatchsetProducedIsVisible(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	do(t, srv, "PUT", "/v1/runs/ps2/steps/1/blob/review", `{"finding":"the lock was never released"}`, nil)
	...
	_ = json.Unmarshal(raw, &es)
	if len(es) != 1 {
		t.Fatalf("what the earlier patchset produced is not visible: %s", raw)
	}
	if es[0].RunID != "ps2" {
		t.Fatalf("which run it came from is not visible: %q", es[0].RunID)
	}
	...
	if len(e4) != 0 {
		t.Fatalf("looked across runs without a scope: %s — the default must be run", raw4)
	}
}

// 심는 것은 계약이 그러라고 할 때뿐이다 (ADR-023 §6.4 자리 3)
func TestLedger_TheListRidesOnlyWithSee(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if _, got := c2["ledger"]; got {
		t.Fatalf("the list rode along unasked: %v", c2["ledger"])
	}
	...
	if !strings.Contains(string(raw), `"first"`) {
		t.Fatalf("see:list yet no list rode along: %s", raw)
	}
}
...
func TestLedger_TheWatermarkStaysInTheSeal(t *testing.T) {
	srv, st := newServerFast(t)
	...
	do(t, srv, "POST", "/v1/nodes/L4/claim", "", nil) // later — first is in the ledger by now
	...
	if len(f.LedgerAt) != 1 || !strings.HasSuffix(f.LedgerAt[0], "-first") {
		t.Fatalf("what was visible was not recorded: %v — "+
			"property 4 demands the sealed bundle alone be enough", f.LedgerAt)
	}
	...
	if len(f1.LedgerAt) != 0 {
		t.Fatalf("the first step saw something: %v", f1.LedgerAt)
	}
}
...
func TestRelease_OnceReleasedAnotherCanTake(t *testing.T) {
	srv, _ := newServerFast(t)
...
// 보고 다음 단계를 시작하지 않는다.
func TestRelease_DropsOutOfTheLeaseList(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if !strings.Contains(string(raw), "rel2") {
		t.Fatalf("the holding node has no lease: %s", raw)
	}
	...
	if strings.Contains(string(raw2), "rel2") {
		t.Fatalf("released, yet still in the lease list: %s", raw2)
	}
}

// ═══ 실행 중 자원 획득 — 실패를 중단이 아니라 값으로 (ADR-022 §7.5 · ADR-024)
//
// I5 의 「요구 자원」은 한 획득 요청의 범위다. t=0 의 requires 는 그 첫 번째
...
func TestAcquire_AMidRunGrabMakesThatNodeRun(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("a1", "first-machine", map[string]string{"role": "x"}), nil)
	do(t, srv, "POST", "/v1/nodes", advert("a2", "later-machine", map[string]string{"role": "y"}), nil)
	...
	if got["try"] != "DONE" {
		t.Fatalf("the acquire step never ran: %q — it goes to no node, so nobody claims it", got["try"])
	}
	if got["fallback"] != "SKIPPED" {
		t.Fatalf("acquired, yet the fallback branch is alive: %q", got["fallback"])
	}
	// t=0 에 없던 노드가 이 Run 에 묶였다
	if code, c := do(t, srv, "POST", "/v1/nodes/a2/claim", "", nil); code != 200 || c["name"] != "on_board" {
		t.Fatalf("the acquired node cannot claim the step: %d %v", code, c)
	}
}

// 못 잡으면 그것도 값이다 — Run 이 죽지 않고 다른 경로로 간다.
// 그리고 이미 쥔 것은 안 놓는다 (부분 점유가 아니라 정상 점유).
func TestAcquire_AFailedGrabTakesTheBranch(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("a3", "present-machine", map[string]string{"role": "x"}), nil)
	// role:y 노드가 함대에 없다.
	...
	if code, _ := do(t, srv, "POST", "/v1/runs", body, nil); code != 201 {
		t.Fatalf("an unobtainable resource blocked submission: %d — "+
			"a mid-run acquire is not matched at t=0", code)
	}
	...
	if got["try"] != "DONE" {
		t.Fatalf("a failed grab was treated as a failure: %q — want a value, not an abort", got["try"])
	}
	if got["on_board"] != "SKIPPED" {
		t.Fatalf("the grab failed, yet that branch is alive: %q", got["on_board"])
	}
	// 이미 쥔 것은 안 놓았다 — fallback 이 첫 노드로 돈다.
	if code, c := do(t, srv, "POST", "/v1/nodes/a3/claim", "", nil); code != 200 || c["name"] != "fallback" {
		t.Fatalf("a failed acquire released what was already held: %d %v — that misreads I5", code, c)
	}
}
...
func TestLoop_TheSpanTurnsUntilTheCondition(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("p1", "a", map[string]string{"role": "x"}), nil)
	...
	if r["rolled_back"] != true {
		t.Fatalf("the condition is unmet yet it did not go back: %v", r)
	}

	// 구간의 첫 단계부터 다시 집힌다 — 회차가 올라간 채로.
	code, c := do(t, srv, "POST", "/v1/nodes/p1/claim", "", nil)
	if code != 200 || c["name"] != "write" {
		t.Fatalf("did not return to the start of the span: %d %v", code, c)
	}
	if got, _ := c["attempt"].(float64); got != 1 {
		t.Fatalf("the attempt did not advance: %v — artifact recency is (attempt, seq)", c["attempt"])
	}
	...
	if r2["rolled_back"] == true {
		t.Fatalf("the condition is met yet it turned again: %v", r2)
	}
}

// 소진하면 그냥 진행한다 — 성패는 success_when 이 정한다 (I3 를 안 건드린다).
func TestLoop_ExhaustionIsProgressNotFailure(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code != 200 || c["name"] != "after" {
		t.Fatalf("did not move on after exhaustion: %d %v", code, c)
	}
	_, v := do(t, srv, "GET", "/v1/runs/lp2", "", nil)
	raw, _ := json.Marshal(v["steps"])
	var steps []struct {
	...
	for _, st := range steps {
		if st.ID == "check" && st.State != "DONE" {
			t.Fatalf("exhaustion was treated as failure: %q — success_when does the judging", st.State)
		}
	}
}
...
// 롱폴이라 계속 돌아서 기존 Run 은 멀쩡하고 새 Run 만 422다.
func TestAdvert_ReturnsTheIntervalInTheResponse(t *testing.T) {
	srv, _ := newServerFast(t)
	_, body := do(t, srv, "POST", "/v1/nodes",
		advert("hb1", "a", map[string]string{"role": "x"}), nil)
	got, ok := body["renew_seconds"].(float64)
	if !ok || got <= 0 {
		t.Fatalf("no interval came: %v — the node would settle it with its own flag",
			body["renew_seconds"])
	}
}
...
// 같은 생 — 재전달
func TestClaim_TheSameLifeAskingAgainGetsTheSameStep(t *testing.T) {
	srv, _ := newServerFast(t)
	life := map[string]string{"X-Enode-Instance": "life-1"}
	...
	if code != 200 || c1["name"] != "one" {
		t.Fatalf("the first claim failed: %d %v", code, c1)
	}
	// 응답이 유실됐다고 치자 — 보고 없이 같은 생이 다시 묻는다.
	code, c2 := do(t, srv, "POST", "/v1/nodes/i1/claim", "", life)
	if code != 200 || c2["name"] != "one" {
		t.Fatalf("the same step did not come back: %d %v — it stays CLAIMED forever", code, c2)
	}
	if c2["seq"] != c1["seq"] || c2["attempt"] != c1["attempt"] {
		t.Fatalf("something else came back: %v vs %v", c1, c2)
	}
	// 실행 정의도 함께 와야 한다 — 이름만 오면 못 돌린다.
	raw, _ := json.Marshal(c2["run"])
	if !strings.Contains(string(raw), "true") {
		t.Fatalf("the redelivery carries no run definition: %s", raw)
	}
	...
	if code, c3 := do(t, srv, "POST", "/v1/nodes/i1/claim", "", life); code != 200 || c3["name"] != "two" {
		t.Fatalf("the next step did not come after the report: %d %v", code, c3)
	}
	// 생이 없으면(옛 enode) 재전달도 없다 — 오늘 그대로.
	if code, c := do(t, srv, "POST", "/v1/nodes/i1/claim", "", nil); code == 200 && c["name"] == "two" {
		t.Fatalf("redelivered without a life: %v", c)
	}
}
...
func TestClaim_ANewLifeFailsTheStepTheOldOneHeld(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	body := contractJSON("inst2", []map[string]any{req("b", map[string]any{"role": "x"})},
		[]map[string]any{runStep("one", "b"), runStep("two", "b")})
	do(t, srv, "POST", "/v1/runs", body, nil)
	do(t, srv, "POST", "/v1/nodes/i2/claim", "", map[string]string{"X-Enode-Instance": "old-life"})

	// 새 생의 하트비트 — 광고에 instance 가 실린다.
	ad, _ := json.Marshal(map[string]any{
		"node_id": "i2", "label": "a", "instance": "new-life",
	...
	if code != 200 {
		t.Fatalf("heartbeat failed: %d", code)
	}
	// 임대 목록이 비어서 온다 — 정산이 임대를 풀었고, 그것이 곧 통보다 (ADR-016).
	raw, _ := json.Marshal(hb["leases"])
	if strings.Contains(string(raw), "inst2") {
		t.Fatalf("a dead run still holds a lease: %s", raw)
	}
	_, v := do(t, srv, "GET", "/v1/runs/inst2", "", nil)
	if v["state"] != "FAILED" {
		t.Fatalf("the run did not die: %v — CLAIMED forever", v["state"])
	}
	...
	if steps[0].State != "FAILED" {
		t.Fatalf("the step that was claimed is %q", steps[0].State)
	}
}
...
func TestReplan_TheContractGrowsTwice(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if code, _ := do(t, srv, "PUT", "/v1/runs/replan1/steps/1/blob/plan",
		...
		t.Fatalf("storing the first plan failed: %d", code)
	}
	...
	if code, cc := do(t, srv, "POST", "/v1/nodes/rp1/claim", "", nil); code != 200 || cc["name"] != "probe" {
		t.Fatalf("the built step did not come: %d %v", code, cc)
	}
	...
	if code, cc := do(t, srv, "POST", "/v1/nodes/rp1/claim", "", nil); code != 200 || cc["name"] != "replan" {
		t.Fatalf("the replan step did not come: %d %v", code, cc)
	}
	...
	if code, cc := do(t, srv, "POST", "/v1/nodes/rp1/claim", "", nil); code != 200 || cc["name"] != "final" {
		t.Fatalf("the step the replan built did not come: %d %v", code, cc)
	}
	...
	if len(m.Contract) != 3 {
		t.Fatalf("%d versions — want three: v1 submitted · v2 plan · v3 replan",
			len(m.Contract))
	}
	...
	if m.Contract[2].By != "step:replan" || len(m.Contract[2].Steps) != 4 {
		t.Fatalf("v3 is wrong: by=%q steps=%d", m.Contract[2].By, len(m.Contract[2].Steps))
	}
	// 첫 판은 「처음 지은 것」이라 바꾼 근거가 없다.
	if len(m.Contract[1].Cause) != 0 {
		t.Fatalf("the first plan carries a cause: %v", m.Contract[1].Cause)
	}
	// 재계획만 「무엇을 보고 바꿨나」를 든다.
	if len(m.Contract[2].Cause) != 1 || !strings.Contains(m.Contract[2].Cause[0], "probe") {
		t.Fatalf("the replan grounds were not recorded: %v — "+
			"the seal alone cannot explain why this path was taken", m.Contract[2].Cause)
	}
}

// 깊이 상한이 종료를 보장한다 — 계약이 아니라 시스템이 쥔다.
func TestReplan_HittingTheCapFailsThatStep(t *testing.T) {
	srv, st := newServerFast(t)
	st.MaxContractVersions = 2 // v1 plus one plan
	...
	_ = json.Unmarshal(sraw, &steps)
	if len(steps) != 2 {
		t.Fatalf("the contract grew past the cap: %d steps", len(steps))
	}
	if steps[1].State != "FAILED" {
		t.Fatalf("the step that hit the cap is %q", steps[1].State)
	}
}
...
func TestWidthCap_SubmissionOverTheCapIs422(t *testing.T) {
	...
	if code, _ := do(t, srv, "POST", "/v1/runs", body, nil); code != 422 {
		t.Fatalf("a requirement over the cap passed: %d", code)
	}
	...
	if code, _ := do(t, srv, "POST", "/v1/runs", both, nil); code != 201 {
		t.Fatalf("one node in two roles was rejected: %d", code)
	}
}

// 실행 중 획득이 넘으면 중단이 아니라 「unavailable」 이다 — 분기로 흐른다.
func TestWidthCap_AnAcquireBlockedByTheCapTakesTheBranch(t *testing.T) {
	srv, _ := newServerFast(t, func(c *config.Config) { c.Lease.MaxPerRun = 1 })
	...
	if got["try"] != "DONE" {
		t.Fatalf("the cap killed the acquire: %q — want a value, not an abort", got["try"])
	}
	if got["got_it"] != "SKIPPED" {
		t.Fatalf("acquired past the cap: got_it=%q", got["got_it"])
	}
	// 이미 쥔 노드로 대체 경로가 돈다
	if code, c := do(t, srv, "POST", "/v1/nodes/w3/claim", "", nil); code != 200 || c["name"] != "without" {
		t.Fatalf("the fallback branch did not come: %d %v", code, c)
	}
}
...
func askStep(id string, answerers []string, timeout map[string]any) map[string]any {
	ask := map[string]any{"prompt": "approve this plan?"}
	if answerers != nil {
...
func TestAsk_TheAnswerDecidesTheBranch(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if raw, _ := json.Marshal(inbox0["asks"]); strings.Contains(string(raw), "ask1") {
		t.Fatalf("asked before prep even finished: %s", raw)
	}
	...
	if code, cc := do(t, srv, "POST", "/v1/nodes/q1/claim", "", nil); code == 200 {
		t.Fatalf("a node claimed the ask: %v", cc)
	}
	_, v := do(t, srv, "GET", "/v1/runs/ask1", "", nil)
	sraw, _ := json.Marshal(v["steps"])
	if !strings.Contains(string(sraw), "ASKED") {
		t.Fatalf("waiting is not visible as a state: %s", sraw)
	}
	...
	if len(asks) != 1 || asks[0].Prompt == "" || len(asks[0].Schema) == 0 {
		t.Fatalf("the inbox does not carry the question: %s", iraw)
	}
	if asks[0].CanAnswer {
		t.Fatalf("outside answerers yet can_answer=true — the viewpoint field is wrong")
	}

	// answerers 집행 — 목록 밖은 403.
	if code, _ := do(t, srv, "POST", "/v1/runs/ask1/steps/2/answer",
		`{"verdict":"approve"}`, nil); code != 403 {
		t.Fatalf("an answer from outside the list passed: %d", code)
	}
	boss := map[string]string{"X-Enode-Principal": "boss@corp"}
	// 스키마 위반은 422 — 질문은 열린 채 남는다
	if code, _ := do(t, srv, "POST", "/v1/runs/ask1/steps/2/answer",
		`{"verdict":"maybe"}`, boss); code != 422 {
		t.Fatalf("an answer violating the shape was stored: %d", code)
	}
	if code, _ := do(t, srv, "POST", "/v1/runs/ask1/steps/2/answer",
		`{"verdict":"approve","note":"looks good"}`, boss); code != 200 {
		t.Fatalf("a valid answer was rejected: %d", code)
	}
	// 두 번째 답은 409 — 이미 답했다.
	if code, _ := do(t, srv, "POST", "/v1/runs/ask1/steps/2/answer",
		`{"verdict":"reject"}`, boss); code != 409 {
		t.Fatalf("a second answer got in: %d", code)
	}

	// 답으로 갈린다 — approve 가 집히고 reject 는 SKIPPED.
	if code, cc := do(t, srv, "POST", "/v1/nodes/q1/claim", "", nil); code != 200 || cc["name"] != "approve" {
		t.Fatalf("the answer did not open its branch: %d %v", code, cc)
	}
	...
	if raw2, _ := json.Marshal(inbox2["asks"]); strings.Contains(string(raw2), "ask1") {
		t.Fatalf("answered, yet still in the inbox: %s", raw2)
	}
}

// 기한 — then:"fail" 사람의 시간은 짐작하지 않되, 선언한 기한은 지킨다.
func TestAsk_APassedDeadlineFails(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if v["state"] != "FAILED" {
		t.Fatalf("the deadline passed yet the run is alive: %v", v["state"])
	}
	// 기한 지난 질문에 답하면 409 — 옛 핸들은 무효다.
	if code, _ := do(t, srv, "POST", "/v1/runs/ask2/steps/1/answer",
		`{"verdict":"approve"}`, nil); code != 409 {
		t.Fatalf("an answer got into a dead question: %d", code)
	}
}
...
func TestGoal_ThePlanProposesAndAPersonAdopts(t *testing.T) {
	srv, st := newServerFast(t)
	...
	// 계획 — 단계 하나와 그 단계를 가리키는 판정 기준을 함께 짓는다.
	...
	if !strings.Contains(string(iraw), `"probe"`) || !strings.Contains(string(iraw), "exit_code") {
		t.Fatalf("the proposal is missing from the inbox: %s — one would approve unseen", iraw)
	}
	...
	if len(m.Contract) != 3 {
		t.Fatalf("%d versions — want three: v1 submitted · v2 plan (proposal) · v3 adopted", len(m.Contract))
	}
	if m.Contract[1].By != "step:plan" || len(m.Contract[1].Proposed) == 0 {
		t.Fatalf("the proposal was not kept in v2: %+v", m.Contract[1])
	}
	if m.Contract[2].By != "answer:gate" || len(m.Contract[2].Cause) == 0 {
		t.Fatalf("v3 is not the version adopted by a human answer: %+v", m.Contract[2])
	}
}

// 거절하면 아무것도 채택되지 않는다 — 제안은 제안으로만 남는다.
func TestGoal_ARejectionGivesTheCriteriaNoForce(t *testing.T) {
	srv, st := newServerFast(t)
	...
	do(t, srv, "POST", "/v1/nodes/g2/claim", "", nil) // probe (ends with exit 1)
	...
	if v["state"] != "SUCCEEDED" {
		t.Fatalf("rejected criteria reached the verdict: %v", v["state"])
	}
	...
	_ = json.Unmarshal(mraw, &m)
	if len(m.Contract) != 2 {
		t.Fatalf("rejected yet %d versions — no adoption version may be attached", len(m.Contract))
	}
}
...
func TestAsk_ShowPutsTheArtifactBodyInTheInbox(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("s1", "a", map[string]string{"role": "x"}), nil)
	...
	gate := map[string]any{
		...
		"ask": map[string]any{"prompt": "please answer the agent's question.",
	...
	do(t, srv, "PUT", "/v1/runs/show1/steps/1/blob/question",
		`{"question":"is this lock ordering intended?"}`, nil)
	do(t, srv, "POST", "/v1/runs/show1/steps/1/result", `{"node":"s1","produced":["question"]}`, nil)

	_, inbox := do(t, srv, "GET", "/v1/asks", "", nil)
	raw, _ := json.Marshal(inbox["asks"])
	if !strings.Contains(string(raw), "is this lock ordering intended?") {
		t.Fatalf("the question body is missing from the inbox: %s", raw)
	}
	...
	bad := contractJSON("show2", []map[string]any{req("b", map[string]any{"role": "x"})},
		[]map[string]any{agentQ, func() map[string]any {
			...
			g["ask"] = map[string]any{"prompt": "?", "show": []string{"nosuch"}}
			return g
		}()})
	if code, _ := do(t, srv, "POST", "/v1/runs", bad, nil); code != 400 {
		t.Fatalf("show names a missing artifact yet got %d", code)
	}
}

// 푸시는 보조다 — 질문이 올라오면 웹훅으로 알리고, 인박스가 정본으로 남는다.
func TestAsk_AQuestionRingsTheWebhook(t *testing.T) {
	...
	case b := <-got:
		...
		if !strings.Contains(e.AnswerPath, "/steps/1/answer") {
			t.Fatalf("no direct answer link: %s", b)
		}
...
//
// produced 는 에이전트가 쓴 파일이고, changed 는 에이전트가 저작하지 않는 관찰이다.
// 아무것도 안 하고 「했다」고 말하는 단계가 여기서 걸린다.

func TestChanged_ChangingNothingFails(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, v := do(t, srv, "POST", "/v1/runs", string(b), nil); code != 201 {
		t.Fatalf("submit failed: %d %v", code, v)
	}
	...
	if !strings.Contains(string(raw), "zImage") {
		t.Fatalf("the paths to check did not ride along: %s", raw)
	}
	...
	if v["state"] != "FAILED" {
		t.Fatalf("changed nothing, yet %v", v["state"])
	}
	vr, _ := json.Marshal(v["verdict"])
	if !strings.Contains(string(vr), "changed") || !strings.Contains(string(vr), "not changed during this step") {
		t.Fatalf("the reason is missing from the verdict: %s", vr)
	}
}

func TestChanged_ReportingAChangePasses(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if v["state"] != "SUCCEEDED" {
		t.Fatalf("changed, yet %v: %v", v["state"], v["verdict"])
	}
}

// 워크스페이스 없는 단계에 changed 를 걸면 400 — 잴 기준이 없다.
func TestChanged_RejectsWhenThereIsNoWorkspace(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, _ := do(t, srv, "POST", "/v1/runs", string(b), nil); code != 400 {
		t.Fatalf("nothing to measure against, yet %d", code)
	}
	// 절대경로·상위참조도 거절한다.
	c["run_id"] = "chg4"
...
// 값은 「왜 못 했나」가 남는 것과, 모델에게 정직할 통로를 주는 것이다.
func TestCannot_AConfessionDoesNotUndoFailure(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if code, _ := do(t, srv, "PUT", "/v1/runs/cannot1/steps/1/blob/_cannot",
		"no toolchain, cannot build", nil); code >= 300 {
		t.Fatalf("the confession was rejected: %d", code)
	}
	do(t, srv, "POST", "/v1/runs/cannot1/steps/1/result",
		`{"node":"n9","produced":["_cannot"],"harness":{"reason":"cannot","message":"no toolchain, cannot build"}}`, nil)

	_, v := do(t, srv, "GET", "/v1/runs/cannot1", "", nil)
	if v["state"] != "FAILED" {
		t.Fatalf("the confession excused the failure: %v", v["state"])
	}
	// 왜 못 했는지가 봉인에 남는다
	raw, err := os.ReadFile(filepath.Join(st.Records.Root, "run-cannot1", "blobs", "01.0-_cannot"))
	if err != nil || !strings.Contains(string(raw), "toolchain") {
		t.Fatalf("the reason is missing from the seal: %v %s", err, raw)
	}
	...
	if !strings.Contains(string(sf), `"cannot"`) {
		t.Fatalf("cannot is missing from the record: %s", sf)
	}
}

// 계약은 _cannot 을 out 으로 요구할 수 없다 — 밑줄 예약이 그것을 막는다.
func TestCannot_AContractCannotUseTheReservedName(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	b, _ := json.Marshal(c)
	if code, _ := do(t, srv, "POST", "/v1/runs", string(b), nil); code != 400 {
		t.Fatalf("the reserved name passed: %d", code)
	}
}
...
func askAdopting(target string) map[string]any {
	return map[string]any{
		"id":  "approve",
		"ask": map[string]any{"prompt": "review it", "adopts": target},
		"out": []string{"approval"},
...
func TestExpands_AnEmptyPlanIsAValue(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, _ := do(t, srv, "PUT", "/v1/runs/exp9/steps/1/blob/plan",
		...
		t.Fatalf("an empty plan was refused storage: %d", code)
	}
	...
	if code, _ := do(t, srv, "POST", "/v1/runs/exp9/steps/1/result",
		...
		t.Fatalf("the step that produced an empty plan failed: %d", code)
	}
	...
	if strings.Contains(string(raw), "ASKED") {
		t.Fatalf("asked for approval on an empty plan: %s", raw)
	}
	if !strings.Contains(string(raw), "SKIPPED") {
		t.Fatalf("that ask is not SKIPPED: %s", raw)
	}
}
...
func TestExpands_AProposalIsValidatedTheMomentItAttaches(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code, c := do(t, srv, "POST", "/v1/nodes/e10/claim", "", nil); code == 200 {
		t.Fatalf("a plan carrying an invalid proposal attached: %v", c)
	}
}

// 사람이 답을 기다리는 동안에는 임대가 만료돼도 Run 을 안 죽인다 (ADR-047)
//
// ADR-032 는 되묻기에 없으면 무한 대기를 못 박았다 —
// "사람의 시간을 시스템이 짐작하지 않는다". 그런데 임대는 기계의 시간으로
...
func TestReap_ARunWaitingOnAnAskIsNotKilled(t *testing.T) {
	srv, st := newServerFast(t)
	...
	if n != 0 {
		t.Fatalf("reaped a run that was waiting on a person: runs=%d", n)
	}
	_, run := do(t, srv, "GET", "/v1/runs/asking", "", nil)
	if run["state"] == "FAILED" {
		t.Fatal("the run died while a person was still looking")
	}
	// 답이 오면 그대로 이어진다 — 예약해 둔 자원이 살아 있다.
	if code, _ := do(t, srv, "POST", "/v1/runs/asking/steps/2/answer",
		`{"verdict":"approve"}`, nil); code >= 300 {
		t.Fatalf("the answer did not get in: %d", code)
	}
	if code, c := do(t, srv, "POST", "/v1/nodes/na/claim", "", nil); code != 200 ||
		c["name"] != "built" {
		t.Fatalf("it did not continue after the answer: %d %v", code, c)
	}
}
...
func TestYolo_AdoptsWithoutAsking(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code != 200 || c["name"] != "built" {
		t.Fatalf("the built step was not claimed: %d %v", code, c)
	}
	...
	if v2["state"] != "SUCCEEDED" {
		t.Fatalf("could not finish without a person: %v", v2["state"])
	}
	// 채택이 봉인에 남았는가 — 계획이 제안한 exit_code 조건이 대조돼야 한다.
	vr, _ := json.Marshal(v2["verdict"])
	if !strings.Contains(string(vr), "exit_code") {
		t.Fatalf("the proposal was not adopted: %s — adopt:\"yolo\" had no effect", vr)
	}
}

// 묻는 것과 안 묻는 것을 함께 선언하지 않는다 (ADR-061 §3)
func TestYolo_TogetherWithAskIsRejected(t *testing.T) {
	srv, _ := newServerFast(t)
	...
	if code != 400 {
		t.Fatalf("a self-contradictory contract passed: %d %v", code, v)
	}
}

// adopts 를 쓰면서 거절이 갈 곳이 없으면 짚는다 (ADR-061 §2)
// 막지는 않는다 — 기존 계약을 깨지 않는다.
func TestAdopts_WarnsWhenARejectionHasNowhereToGo(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("w1", "a", map[string]string{"role": "x"}), nil)
	gate := adoptingGate("approve", "plan")
	delete(gate, "dispatch") // the third-run family looks like this
	body, _ := json.Marshal(map[string]any{
		"run_id":   "warn1",
		"requires": []map[string]any{req("b", map[string]any{"role": "x"})},
	...
	code, v := do(t, srv, "POST", "/v1/runs", string(body), nil)
	if code != 201 {
		t.Fatalf("the warning blocked submission: %d %v", code, v)
	}
	raw, _ := json.Marshal(v["warnings"])
	if !strings.Contains(string(raw), "rejecting the plan would do nothing") {
		t.Fatalf("the warning did not ride along: %s", raw)
	}
}
...
func TestReject_ThePlanRetiresAndTheReplanReusesTheNames(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("rp1", "a", map[string]string{"role": "x"}), nil)
	...
	if code != 200 || c["name"] != "again" {
		t.Fatalf("the replan was not claimed: %d %v", code, c)
	}
	// 여기가 핵심이다 — 거절당한 계획과 같은 이름으로 다시 짓는다.
	if code, v := do(t, srv, "PUT", "/v1/runs/rej9/steps/4/blob/plan2",
		`{"steps":[{"id":"work","uses":"b","run":["true"],"out":["work"]}]}`, nil); code >= 300 {
		t.Fatalf("storing the plan failed: %d %v", code, v)
	}
	if code, v := do(t, srv, "POST", "/v1/runs/rej9/steps/4/result",
		`{"node":"rp1","produced":["plan2"]}`, nil); code != 200 {
		t.Fatalf("report failed: %d %v", code, v)
	}
	_, v := do(t, srv, "GET", "/v1/runs/rej9", "", nil)
	raw, _ := json.Marshal(v["steps"])
	if strings.Contains(string(raw), "\"state\":\"FAILED\"") {
		t.Fatalf("the replan died on a name collision: %s", raw)
	}
	code, c2 := do(t, srv, "POST", "/v1/nodes/rp1/claim", "", nil)
	if code != 200 || c2["name"] != "work" {
		t.Fatalf("the rebuilt step was not claimed: %d %v", code, c2)
	}
}

// 미룬 검사가 계획이 붙을 때 실제로 돈다 (ADR-062)
//
// dispatch.to 가 약속한 이름을 가리키면 제출 시점에는 DAG 와 도달 가능성을
// 볼 수 없다 — 그 단계가 아직 없다. 면제가 아니라 미루는 것 이므로,
// 계획이 그것을 뒤가 아닌 자리에 지으면 그때 거절돼야 한다.
func TestDispatch_RejectsAPromisedDestinationBuiltTooEarly(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("d1", "a", map[string]string{"role": "x"}), nil)
	plan := planStep("b")
	plan["produces"] = []string{"target"}
	gate := map[string]any{
		"id": "gate", "needs": []string{"plan"}, "uses": "b",
		"agent": map[string]any{"ask": "never"}, "out": []string{"route"},
		"schema": map[string]any{"route": map[string]any{
			"type": "object", "required": []string{"next"},
			"properties": map[string]any{
				"next": map[string]any{"enum": []string{"target", "other"}}}}},
		"dispatch": map[string]any{"from": "route.next", "to": []string{"target", "other"}},
	}
	other := runStep("other", "b")
	other["needs"] = []string{"gate"}
	body, _ := json.Marshal(map[string]any{
		"run_id":       "dsp1",
		"requires":     []map[string]any{req("b", map[string]any{"role": "x"})},
		"steps":        []map[string]any{plan, gate, other},
		"success_when": []map[string]any{{"step": "target", "produced": []string{"target"}}},
	})
	// 제출은 통과한다 — target 은 plan 이 약속했다.
	if code, v := do(t, srv, "POST", "/v1/runs", string(body), nil); code != 201 {
		t.Fatalf("submission was blocked on a promised name: %d %v", code, v)
	}
	do(t, srv, "POST", "/v1/nodes/d1/claim", "", nil)
	// 계획이 target 을 짓는다 — 새 단계는 늘 뒤에 붙으므로 DAG 는 만족한다.
	// 그러나 needs 를 other 에 매달면 도달할 수 없다: gate 가 target 을
	// 고르는 순간 other 가 SKIPPED 가 되고 target 이 그 뒤에 매달려 있다.
	do(t, srv, "PUT", "/v1/runs/dsp1/steps/1/blob/plan",
		`{"steps":[{"id":"target","uses":"b","run":["true"],"out":["target"],"needs":["other"]}]}`, nil)
	if code, _ := do(t, srv, "POST", "/v1/runs/dsp1/steps/1/result",
		`{"node":"d1","produced":["plan"]}`, nil); code != 200 {
		t.Fatalf("report failed: %d", code)
	}
	// 여기가 핵심이다 — 미룬 도달 가능성 검사가 그 단계를 거절해야 한다.
	_, v := do(t, srv, "GET", "/v1/runs/dsp1", "", nil)
	raw, _ := json.Marshal(v["steps"])
	if !strings.Contains(string(raw), `"state":"FAILED"`) {
		t.Fatalf("an unreachable plan passed: %s — the deferred check never ran", raw)
	}
}

// 거절은 분기가 아니라 되돌림이다 (ADR-062)
//
// 계약이 "승인하면 여기로 간다" 를 적으려면 그 자리가 계획 안 어디인지를
// 알아야 하는데, 그것은 계획을 짓는 쪽만 안다 — 앞단이 뒷단의 모양을 단정하면
// 승인해도 못 닿는 그래프가 나온다(실측 promised-1).
//
// ⇒ 계약은 목적지를 안 적는다. 승인이면 계획대로 이어서 돌고, 거절이면
//
//	계획을 지은 단계로 돌아가 다시 짓는다.
func TestReject_RewindsToThePlanStep(t *testing.T) {
	srv, _ := newServerFast(t)
	do(t, srv, "POST", "/v1/nodes", advert("rw1", "a", map[string]string{"role": "x"}), nil)
	plan := planStep("b")
	plan["produces"] = []string{"work"}
	gate := map[string]any{
		"id": "approve", "needs": []string{"plan"},
		// dispatch 가 없다 — 계약은 갈 곳을 지목하지 않는다.
		"ask": map[string]any{"prompt": "?", "adopts": "plan", "adopt_when": "ok"},
		"out": []string{"decision"},
		"schema": map[string]any{"decision": map[string]any{
			"type": "object", "required": []string{"verdict"},
			"properties": map[string]any{
				"verdict": map[string]any{"enum": []string{"ok", "again"}}}}},
	}
	body, _ := json.Marshal(map[string]any{
		"run_id":       "rw1",
		"requires":     []map[string]any{req("b", map[string]any{"role": "x"})},
		"steps":        []map[string]any{plan, gate},
		"success_when": []map[string]any{{"step": "work", "exit_code": 0}},
	})
	if code, v := do(t, srv, "POST", "/v1/runs", string(body), nil); code != 201 {
		t.Fatalf("submit failed: %d %v", code, v)
	}
	buildPlan := func(blob, id string) {
		t.Helper()
		do(t, srv, "POST", "/v1/nodes/rw1/claim", "", nil)
		do(t, srv, "PUT", "/v1/runs/rw1/steps/1/blob/"+blob,
			`{"steps":[{"id":"`+id+`","uses":"b","run":["true"],"out":["`+id+`"]}]}`, nil)
		do(t, srv, "POST", "/v1/runs/rw1/steps/1/result", `{"node":"rw1","produced":["`+blob+`"]}`, nil)
	}
	// 1 회차 — 지었다가 거절당한다
	buildPlan("plan", "work")
	if code, _ := do(t, srv, "POST", "/v1/runs/rw1/steps/2/answer",
		`{"verdict":"again"}`, nil); code != 200 {
		t.Fatalf("reject failed: %d", code)
	}
	// 계획 단계가 다시 집혀야 한다
	code, c := do(t, srv, "POST", "/v1/nodes/rw1/claim", "", nil)
	if code != 200 || c["name"] != "plan" {
		t.Fatalf("did not rewind to the plan step: %d %v", code, c)
	}
	// 2 회차 — 같은 이름으로 다시 짓는다
	do(t, srv, "PUT", "/v1/runs/rw1/steps/1/blob/plan",
		`{"steps":[{"id":"work","uses":"b","run":["true"],"out":["work"]}]}`, nil)
	if code, v := do(t, srv, "POST", "/v1/runs/rw1/steps/1/result",
		`{"node":"rw1","produced":["plan"]}`, nil); code != 200 {
		t.Fatalf("the second attempt report failed: %d %v", code, v)
	}
	// 이번엔 승인한다
	do(t, srv, "POST", "/v1/nodes/rw1/claim", "", nil) // approve
	if code, _ := do(t, srv, "POST", "/v1/runs/rw1/steps/2/answer",
		`{"verdict":"ok"}`, nil); code != 200 {
		t.Fatalf("approve failed: %d", code)
	}
	code, c2 := do(t, srv, "POST", "/v1/nodes/rw1/claim", "", nil)
	if code != 200 || c2["name"] != "work" {
		t.Fatalf("the plan does not run after approval: %d %v", code, c2)
	}
}
