...
#
#   scripts/nested-runc-overlay-probe.sh [--mount-at PATH] [--ssh-dir PATH] ROOTFS LOWER SCRATCH [COMMAND]
#
...
usage() {
  cat <<'EOF'
usage: nested-runc-overlay-probe.sh [--mount-at PATH] [--ssh-dir PATH] ROOTFS LOWER SCRATCH [COMMAND]
...
--ssh-dir PATH  호스트의 .ssh 디렉터리 전체를 rootfs uid 1000 사용자의
                 ~/.ssh에 읽기 전용으로 탑재한다. config, known_hosts, private key를 포함한다.
                 ROOTFS 안에 uid/gid 1000 소유의 .ssh 디렉터리를 미리 만들어야 한다.
...
# SSH host alias와 key가 필요한 private repo:
  scripts/nested-runc-overlay-probe.sh --ssh-dir "$HOME/.ssh" \
    "$HOME/rootfs" /srv/yocto "$HOME/ovl-probe"

# --ssh-config FILE은 기존 호출을 위한 별칭이며 FILE의 부모 디렉터리를 탑재한다:
...
require_ssh_dir_target() {
  local ssh_target="$1"

  [ -d "$ssh_target" ] && [ -x "$ssh_target" ] && [ -O "$ssh_target" ] && [ -G "$ssh_target" ] || \
    fail "rootfs SSH 대상은 uid/gid 1000 소유의 접근 가능한 디렉터리여야 한다: sudo install -d -o 1000 -g 1000 -m 700 '$ssh_target'"
}
...
if [ -n "$SSH_CONFIG_ALIAS" ]; then
  [ -z "$SSH_DIR" ] || fail "--ssh-dir와 --ssh-config를 함께 쓸 수 없다"
  SSH_CONFIG_ALIAS="$(realpath "$SSH_CONFIG_ALIAS")" || fail "SSH config 경로를 해석할 수 없다"
  [ -f "$SSH_CONFIG_ALIAS" ] || fail "SSH config가 일반 파일이 아니다: $SSH_CONFIG_ALIAS"
  SSH_DIR="$(dirname "$SSH_CONFIG_ALIAS")"
fi
if [ -n "$SSH_DIR" ]; then
  SSH_DIR="$(realpath "$SSH_DIR")" || fail "SSH 디렉터리 경로를 해석할 수 없다"
  [ -d "$SSH_DIR" ] || fail "SSH 디렉터리가 아니다: $SSH_DIR"
  [ -r "$SSH_DIR" ] && [ -x "$SSH_DIR" ] || fail "SSH 디렉터리를 읽을 수 없다: $SSH_DIR"
fi
...
if [ -n "$SSH_DIR" ]; then
  ...
  SSH_DIR_DEST="$rootfs_home/.ssh"
  require_ssh_dir_target "$ROOTFS$SSH_DIR_DEST"
fi

PROBE_ROOTFS="$ROOTFS" \
PROBE_MERGED="$MERGED" \
PROBE_MOUNT_AT="$MOUNT_AT" \
PROBE_SSH_DIR="$SSH_DIR" \
PROBE_SSH_DIR_DEST="$SSH_DIR_DEST" \
...
ssh_dir = os.environ["PROBE_SSH_DIR"]
if ssh_dir:
    config["mounts"].append(
        {
            "destination": os.environ["PROBE_SSH_DIR_DEST"],
            "type": "bind",
            "source": ssh_dir,
...
printf 'ssh_dir=%s\n' "$([ -n "$SSH_DIR" ] && echo mounted-ro || echo absent)"
