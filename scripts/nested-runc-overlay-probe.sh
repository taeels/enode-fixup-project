...
#
#   scripts/nested-runc-overlay-probe.sh ROOTFS LOWER SCRATCH [COMMAND]
#
# ROOTFS  Ubuntu rootfs. /bin/bash와 미리 만든 /work가 있어야 한다.
# LOWER   읽기 전용 아래 층으로 쓸 준비된 워크스페이스.
# SCRATCH upper/work/merged와 결과를 둘 디렉터리. Docker overlay2가 아닌
#         호스트 bind volume을 줘야 한다.
# COMMAND runc 안에서 /work를 cwd로 실행할 셸 한 줄. 생략하면 쓰기만 검사한다.
#
...
usage() {
  cat <<'EOF'
usage: nested-runc-overlay-probe.sh ROOTFS LOWER SCRATCH [COMMAND]
...
SCRATCH="$(realpath "$3")"
RUN_COMMAND="${4:-printf 'probe-write-ok\n' > /work/.enode-write-test; rm /work/.enode-write-test; echo COMMAND_OK}"

[ -d "$ROOTFS" ] || fail "rootfs가 디렉터리가 아니다: $ROOTFS"
[ -x "$ROOTFS/bin/bash" ] || fail "rootfs에 /bin/bash가 없다: $ROOTFS"
[ -d "$ROOTFS/work" ] || fail "rootfs에 /work가 없다: sudo install -d -m 0755 '$ROOTFS/work'"
...
wrapper = """set -e
start_ns=$(cat /work/.enode-probe-start-ns)
now_ns=$(date +%s%N)
...
config["mounts"] = [
    ...
    if mount.get("destination") not in ("/sys", "/sys/fs/cgroup", "/work")
]
