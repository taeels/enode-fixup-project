...
그래서 눈으로 확인했다. **두 핀 모두 아무것도 켜지지 않는다.**

```text
   온보드 ACT (GPIO47)   2Hz 10초   보인다      대조군
   온보드 PWR (GPIO35)   2Hz 10초   보인다      대조군
   GPIO 17               2Hz  5초   안 보인다
   GPIO 27               2Hz  5초   안 보인다
```

대조군이 이 측정의 전부다. "안 보인다" 만으로는 LED 가 없다는 뜻인지 보는
사람이 딴 데를 보고 있다는 뜻인지 안 갈린다. 그래서 **보이는 것이 확실한
온보드 두 개를 같은 조건에서 먼저 흔들어** 관측이 성립하는 것을 확인한 뒤에
바깥 핀을 걸었다. 온보드는 보이고 17·27 은 안 보였다.

전기적으로도 재 봤지만 그쪽은 답을 못 준다. 핀을 입력으로 두고 내부 풀업과
풀다운을 걸어 레벨을 읽으면 17 과 27 은 풀을 그대로 따라간다. 그런데
**LED 가 확실히 달린 GPIO47 도 똑같이 따라간다.** 내부 풀업은 수십 µA 라
LED 순방향 전압에 못 미쳐서 부하가 안 보이기 때문이다. 이 시험으로는 LED
유무를 못 가른다는 것을 GPIO47 이 증명한다.
...
이걸로 한 번 헛돌았다. 위의 전기 측정에서 `pinctrl set 47 ip` 로 GPIO47 을
입력으로 바꾼 뒤 17 과 27 만 출력으로 되돌리고 **47 과 35 는 그대로 두었다.**
그 뒤로 ACT 는 아무리 켜도 안 켜졌다.

```text
   echo 1 > /sys/class/leds/ACT/brightness
   cat  /sys/class/leds/ACT/brightness      ->  0     썼는데 0 으로 읽힌다
```

핀이 입력으로 묶여 있으면 LED 클래스가 값을 받아도 핀을 못 몬다. 읽기가
`0` 으로 돌아오는 것이 그 신호다. PWR 도 같이 죽어서 전원 표시가 꺼진 채로
남았다. **그 상태에서 돌린 시험은 전부 무효였다** — 안 보인 것이 LED 탓인지
핀 탓인지 갈리지 않는다.

`pinctrl set 47 op` 와 `pinctrl set 35 op` 로 되살아난다. 애초에 온보드 LED
는 `/sys/class/leds` 만으로 충분하므로 그 핀에는 `pinctrl` 을 대지 않는 것이
맞다. 이 recipe 의 스크립트는 그래서 `pinctrl` 을 아예 안 쓴다.
