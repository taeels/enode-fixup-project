...
**Run 흐름·보기 설정 보완**: [후속 계획](../../plans/ui-run-flow-plan.md)에 따라
상단 네 버튼을 화면·표현 드롭다운으로 교체했다. RunScene은 게스트/제출자와
Mediator 역할 요소를 실제 단계 DAG 앞에 표시하고 SceneState.actor로 선택한다.
이 역할은 API 노드나 실행 단계가 아니다. 좁은 Run 화면은 세로 배치·폭 맞춤과
일반 휠/터치 스크롤을 사용하고 Ctrl/Cmd+휠·버튼으로 확대한다. 넓은 화면·함대의
기존 휠 동작과 전체 ID는 유지한다. 아래의 이전 버튼 목록은 생성 당시 기록이며
현재 제어와 검증은 [구현 결과](../code/ui-run-flow.md)를 따른다.

**긴 이름 추가 피드백**: 함대 배치 간격을 넓히고 노드 이름을 최대 두 줄로 표시한다.
전각 문자 폭을 반영하며 전체 이름은 접근성 이름·title·상세에 보존한다.

**2026-09-09 후속 보완**: [함대 탐색·정보 가독성 계획](../../plans/ui-readability-plan.md)을
승인된 코드 생성 중 사용자 개선으로 적용했다. 함대 장치는 유지하고 작업 그래프는
순번·상태·용도·실행 노드가 있는 단계 카드와 needs 화살표로 표시한다. 3D는 카드의
깊이와 기존 등각 배치를 사용한다. 그래프 헤더에서 함대로 돌아가면 이전 배율과
스크롤을 복원하며 목록 선택·필터는 유지한다.

장면 내부 휠은 포인터 기준 확대·축소, 가로 휠과 Shift+휠은 이동이다. 정보 카드와
Run 목록의 휠은 읽기 스크롤이다. 정보 카드는 현재 작업의 제출자, 능력별 속성 표,
연결 시각, 접힌 기술 상세를 제공한다. 기술 상세의 펼침·포커스·스크롤을 갱신 중
유지한다. `*-graph-back-button`과 `*-node-technical-toggle`은 키보드 조작을 제공한다.
