...
| submitter | guest.guestName()의 한글 두 단어. `^[가-힣]{1,12} [가-힣]{1,12}$`, 최대 UTF-8 73바이트. 이전 요청 재시도 호환으로 기존 영문 Guest 형식(최대 55 ASCII 바이트)도 허용. 인증 신원 아님 |
...
기존 영문 사전은 한글 사전으로 옮기며 두 번째 이름 입력 폼은 추가하지 않는다.
기존 영문 저장 이름은 결정적으로 한글로 이전한다. 저장된 미확인 요청은 이름을
포함한 원래 세 값을 유지한다. 서버도 받은 원문으로 식별을 계산하며 번역하지 않는다.
...
| 409 | queue 인수 뒤 일반 점유 실패는 202다. 예외적인 409가 오면 충돌로 표시하고 QUEUED로 꾸미지 않음 |
