...
- [ ] **9 — 실제 데모 연결** (선행: 실제 픽스처·demo-back·방송 입력)
  ...
  - [x] 실제 LED/음원 계약 c7a237d와 demo-back POST/GET·이름·선택 연결.
    로컬 실제 API/DB의 두 시나리오 접수·queue 승격·브라우저 14개 단언 통과.
...
[함대 탐색·정보 가독성 보완](ui-readability-plan.md)은 1·2·5·10단계의 추가 개선이다.
휠 확대·축소, 작업 단계 카드·함대 복귀, 현재 임대의 제출자 표시와 읽기 쉬운
정보 카드·시각을 구현한다. 이 보완으로 기존 공동 장면 보류를 해제하지 않는다.
