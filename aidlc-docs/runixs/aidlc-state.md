...
승인된 UI 계획 1단계부터 실행한다. 공용 입력 확인은 해당 연결 단계의 조건으로 유지한다.
...
검토를 위해 작성한 코드 계획은 이제 승인됐다. 코드 생성은 진행 중이며
신규 기능의 Build and Test·장면 완료는 아직 아니다. 기존 코드
테스트·브라우저 검증은 재사용 판단을 위한 사전 검증이다. 승인된 Inception은 유지한다.
