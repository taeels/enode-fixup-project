...
```text
   A (권장)  unit/<유닛> 에서 돌고 게이트가 초록이면 v3-run-harness-components 로 병합.
             팩이 다 끝난 뒤 회차 브랜치를 PR 로 main 에 올린다.
             CONVENTIONS 3.1 의 「실행은 이 둘 위에서 끝난다」 그대로다
...
짝 팩이 그 위에」를 정했고, 그 순서는 **팩 단위**로 매겨진 것이다. B 로 유닛마다
`main` 에 올리면 짝 팩이 `main` 을 따라오는 도중에 `runner.go` 가 다섯 번
부딪힌다. C 는 게이트가 빨간 채로 커밋이 쌓여도 되돌릴 경계가 없다 —
