...
```text
   ...
   errAux 로 감싼 오류          보조.  enode-settings.json 의 Marshal · WriteFile 둘.
...
| SECURITY-12 자격증명 | **재판정 (⑮ 뒤)** | 복사본은 `0600` · 계장 디렉터리 안 · 단계 끝 삭제 — **디스크 경계는 지킨다**. 광고와 허용목록에는 이름만. 그러나 SECURITY-03 행이 적은 대로 **봉인 기록으로 가는 문이 새로 열렸다.** 디스크에서 지우는 것과 기록에 안 남는 것은 다른 값이고, 이 회차는 앞만 값으로 닫았다 |
...
| SECURITY-03 로깅 | **재판정 (⑮ 뒤)** | 노드 로그는 이름만 찍는다 (`components.md` 4절). **그런데 ⑮ 가 `logs/` 의 내용을 바꾼다** — `stream-json` 아래 stdout 은 봉투 하나가 아니라 사건 전부이고 거기 도구 입력과 도구 결과가 실린다. `logs/NN-*.log` 는 `ADR-005` 성질 2 로 봉인되어 이후 안 바뀐다. 에이전트가 가짜 홈의 `.credentials.json` 을 한 번 읽으면 그 내용이 봉인 기록에 영구히 남는다 — ⑩ 이 디스크 경계로 막은 값을 다른 문으로 낸다. **U1 의 Functional Design 이 닫는다** — 하네스에 그 파일을 안 보이게 하거나(권한), 사건을 거르거나, 위험을 값으로 받아들이고 적는다 |
