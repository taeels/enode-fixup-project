...
```text
   ...
   실패 등급        errComponents 로 감싼 오류만 치명이다.  훅 실패는 오늘 그대로 보조
   Fixed           h.Fixed(tmp) 로 부른다
```

### 1.4 탐지 — `internal/enode/detect.go` · `detector.go`

`Detector`(시계)는 **안 건드린다.** `ADR-035` §4.2 가 지키려던 값은
`ADR-068` 이 이미 세웠다 — `Detector.Capabilities()` 가 외부 프로세스를 안
띄우므로 광고 루프가 탐지에서 멈출 수 없다.

```text
   costlyAttrs     mcpAttrs(l, log) 하나를 더해 합친다 (Q3 = A).  새 타입 0
   하네스 순회      break 를 걷는다.  harness.<이름>: "1" 을 싣고
                   옛 harness: <이름> 도 이 회차 동안 같이 싣는다
   cheapAttrs      안 건드린다
```
...
**책임** — 「지금 뜨겠는가」 하나. 프로세스를 안 띄운다. 그래도 비싼 쪽에 앉는
이유는 `PATH` 를 훑기 때문이고, 그것이 `requirements.md` 4.4 의 확정이다.
...
| 가짜 홈 짓기 | 디렉터리 · 파일 쓰기 오류 | **치명** | `errComponents` 로 감싼다 |
...
| 훅 설정 쓰기 | 오류 | 보조 | 오늘 그대로. 진짜 안전망은 워크스페이스 diff 다 |
| 기준 시각 파일 | 오류 | 보조 | 오늘 그대로 |
