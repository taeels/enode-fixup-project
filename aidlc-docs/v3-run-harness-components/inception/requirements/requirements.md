...
| SECURITY-03 애플리케이션 로깅 | 새 표면 | 새 로그는 이름만 찍는다 — 빠진 MCP 의 이름 · 겹친 이름 · 팩에서 무시한 파일. 값은 안 찍는다. **`logs/` 도 이 규칙의 대상이다** — `runner.go` 가 stdout 을 원문으로 싣고 그 묶음은 `ADR-005` 성질 2 로 봉인된다. ⑮ 이 그 표면을 넓혔고 **⑱ 이 닫는다** — 도구 사건의 내용을 걷고 껍데기와 `init` 줄과 최종 봉투와 stderr 를 남긴다. 경로에 예외가 없다 |
...
| SECURITY-11 보안 설계 | 새 표면 | 겹이 둘이다 — 가짜 홈과 strict 허용목록. **첫 겹은 로그아웃 홈에서만 쟀다** — CA1 이 로그인된 홈에서 다시 잰다 (`application-design.md` 4.6). 오용 시나리오는 4.3 |
| SECURITY-12 인증 · 자격증명 | N/A | 그 규칙은 사용자 인증(비밀번호 정책 · MFA · 세션)이라 계장의 파일 복사에 안 걸린다. 복사본의 `0600` · 단계 끝 삭제 · 임시 디렉터리 밖으로 안 나감은 `features.md` 3.1 의 보안 요구가 받는다. 허용목록과 광고에는 이름만. R1 화이트리스트는 그대로 — `ENODE_TOKEN` 은 안 넘어간다 |
