...
문서는 **회차(브랜치) 하나**로 모은다. Inception 과 Construction 산출물이 그 회차의
폴더에 함께 쌓인다. 회차가 다르면 두 손이 같은 파일을 두고 안 부딪친다.
...
```text
   ...
   ├── <브랜치 이름>/                회차 하나의 산출물 — INCEPTION 과 CONSTRUCTION
   ...
   │       ├── <유닛>/
   │       └── build-and-test/
   ├── <handle>/                     앞 회차(v1 ~ v3)의 Construction 기록.  새로 안 쓴다
...
담당 루트가 있는 handle — `shin-son` · `nacl1119` · `taeels` · `runixs`. 2026-09-24
굽기 회차부터 Construction 도 회차 폴더에 쓴다.

## 규칙 셋

- **문서 루트는 회차 하나다** — Inception 과 Construction 모두
  `aidlc-docs/<브랜치 이름>/`. 루트 `aidlc-docs/` 에 상태·감사·단계 문서를 다시
  쓰지 않는다. 회차끼리 같은 파일을 두고 부딪치기 때문이다 (`CLAUDE.md` 의 문서
  루트 규약).
- **RE 만 공용** — `reverse-engineering` 은 회차와 무관하게 참이라 한 벌만 둔다.
- **자기 것은 자기가 고친다** — `aidlc-state.md` 는 자동 병합이 안 되지만 회차마다
  하나라 소유자(그 회차의 진행자)가 하나다. 그 소유자가 고치고 자기 브랜치에 싣는다. `audit.md` 는
