...
**W-b 가 닫혔다 (2026-09-16T05:40:00Z · 사용자 「w-b pr둘고 메인에 올리고 w-c 진행」).**
유닛 둘의 단계가 다 돌았고 PR 셋으로 `main` 에 갔다 — #42 회차 · #43 U2 · #44 U4.
병합 전에 **합친 나무에서** 게이트를 다시 쟀다 (시험 19 패키지 초록 · 스킵 0 ·
커버리지 미달 0 · **라우트 18 — CB0 의 가르는 값**). 값과 남긴 것은 담당 루트
`aidlc-docs/taeels/aidlc-state.md` 의 「W-b 가 닫혔다」.

**NFR 단계 둘이 회차 계획과 갈렸다** — 계획은 U4 를 EXECUTE 로 걸고 N1 을 지게
했는데 사용자가 전부 스킵을 지시했다. N1 은 안 닫히고 **봉투를 산수로** 남았다
(U4 의 Code Generation 계획 0절). 아픈 자리가 데모의 전역 한도 120 req/s 다 —
이 폴링 라우트가 `GET /v1/nodes` · `GET /v1/runs` 와 한 바구니를 나눠 쓴다.
한도는 안 바꾸고 **U8 에 「보이는 카드만 폴링한다」를 넘겼다.**
