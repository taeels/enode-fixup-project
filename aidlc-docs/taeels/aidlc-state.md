...
## W-d 를 뒤늦게 연다 — U6 `panel-past` (2026-09-17)

**이 회차의 마지막 구멍이다.** 게이트 일곱 중 CB2 하나가 유닛이 없어 안 섰고,
U8 의 CB6 이 그것 때문에 보류다. 사용자가 「U6 이 먼데」로 묻고 「그래」로 열었다.

브랜치 `unit/panel-past` 를 **`unit/fleet-card` 에서 땄다** — 물음 1 = A 다.
앞선 유닛들이 `main` 에서 딴 것과 다르고, 이유는 아래 ②다.

```text
   ①  출처만 바꾸면 화면이 안 바뀐다   page.go 의 loadRecord 가 바이트를 그대로
                                  <pre> 에 붓는다.  tar 에서 꺼낸 바이트 대신
                                  GET 이 준 같은 바이트를 같은 <pre> 에 넣는
                                  것이라 CB2 의 「같은 파서의 같은 모양」이 안 선다.
                                  **유닛 정본의 경로 줄에 page.go 가 없다**

   ②  U6 의 의존이 바뀌었다          U8 이 렌더러를 window.enodeCard 로 옮겨 놨다.
                                  U6 이 그것을 부르면 **그리는 코드가 0 줄**이다.
                                  의존 행렬에 U8 칸이 없는 것은 그 행렬이
                                  2026-09-17 의 설계 변경보다 앞서기 때문이다
```

**실측이 숫자 하나를 냈다** — `cb4-card-4` 의 `survey` 는 진행 파일 127,117
바이트 대 봉인된 로그 11,553 바이트로 **9%** 이고, 걷힌 양이 `enode.elided` 한
줄에 `{"events":98,"bytes":408989}` 로 있다. **봉인된 것은 도는 동안 본 것과
다른 물건이다.**
...
```text
   Functional Design      Part 1 (계획) 2026-09-17.  계획 construction/plans/
                          panel-past-functional-design-plan.md
                          실측 여덟 · 물음 여섯 · **답 여섯이 전부 A**
                          (사용자 「권장대로」)
                          산출물 construction/panel-past/functional-design/
                          규칙은 U8 의 R61 에 이어 **R62 ~ R89**
                          승인 2026-09-17 (사용자 「승인. 코드 써라.」)

   NFR Requirements       **SKIP.**  unit-of-work.md 10절이 걸었다 — 출처만
   NFR Design             바뀌고 기존 do() 를 탄다.  앞의 U4 · U5 · U8 과 달리
   Infrastructure Design  **어긋남이 아니다** (그 셋은 정본이 「돈다」였다)

   Code Generation        Part 1 (계획) 2026-09-17.  계획 construction/plans/
                          panel-past-code-generation-plan.md
                          **Step 여덟 · 체크박스 쉰셋 · 물음 0**
                          실측 다섯 · 앞의 셋이 설계의 줄을 고친다 (아래)
                          승인 2026-09-17 (사용자 「다음으로」)

                          Part 2 (생성) 2026-09-17.  요약 construction/panel-past/
                          code/code-summary.md
                          체크박스 **쉰셋 중 쉰둘** — 남은 하나는 CB2 (사람)
                          **새 파일 0 · 새 Go 패키지 0.**  출처만 바꾼다
                          변이 일곱이 전부 죽었다
                          기계가 재는 게이트 전부 초록 (6절)
                          **승인 대기**
```

### 계획 단계의 실측이 설계의 줄 셋을 고쳤다

```text
   ①  봉투 하나가 두 읽기에서   설계가 단계마다 as=events 와 as=raw 를 따로
      나오면 R69 가 깨진다      부르기로 적었다.  봉인 뒤는 파일이 불변이라
                              안 아프고 **봉인 전은 진행 파일이 자라서 사건
                              열과 원문이 다른 창을 보인다.**  U5 의 liveBody
                              주석이 같은 자리를 이미 적어 뒀다.
                              고침 — as=raw 를 한 번 부르고 제어판이
                              transcript.Parse 를 부른다.  서버의 as=events 가
                              하는 일이 정확히 그것이고 from 을 안 쓰므로
                              headCut 이 거짓이라 **글자까지 같은 답**이다.
                              요청이 2N+1 에서 **1+N** 으로 준다 — 설계 1.1 의
                              산수가 그 수였다.  두 줄이 어긋나 있었다

   ②  sealed 를 낼 독립된      source 에서 유도하면 「같은 답을 내야 한다」가
      출처가 없다              자동으로 참이라 재는 값이 0 이고, Terminal(state)
                              로 내면 **봉인 창에서 거짓말을 한다.**
                              고침 — 봉투에서 빼고 Run 의 state 를 싣는다.
                              봉인 전과 뒤는 단계마다의 source 가 말한다
                              (R73 · R74 가 이미 단계 단위다)

   ③  한 응답 상한 1 MiB 가    폴링하는 U8 은 from 으로 이어 받고 이 유닛은
      지난 것에도 걸린다        한 번 읽고 끝이라 **나머지가 영영 안 온다.**
                              봉인된 로그가 넘으면 capped 헤더도 안 붙는다.
                              고침 — 화면이 total 과 받은 길이를 견주어
                              「처음 N 바이트만 왔다」를 적는다
```

**셋 다 승인받은 답 여섯을 안 무른다.** 계획의 승인이 이 고침의 승인을 겸한다.

### 코드가 섰다 — 계획의 한 줄을 실측이 다시 고쳤다

**`internal/panel` 의 tar 를 `go list -deps` 로 못 잰다.** `internal/enode` 가
`archive/tar` 를 딛고(봉인이 tar 를 짓는다) 제어판이 그것을 임포트하므로
의존 그래프에는 언제나 보인다. **직접 임포트**를 재는 것으로 고쳤다.
「파일에 tar 라는 낱말이 0」도 안 지켰다 — 남은 넷이 전부 **걷힌 이유를 적는
주석**이고, 낱말 수는 임포트가 사라졌는지의 대용이었다.

**봉투에 `received` 를 더했다.** 계획 1.3 이 「화면이 `total` 과 받은 길이를
견준다」였는데 **브라우저가 그 길이를 못 센다** — JS 문자열의 길이는 UTF-16
단위라 하네스가 한국어를 한 줄만 내도 바이트 수와 갈린다. 서버가 센다.

**변이 ④ 를 재려고 시험의 픽스처를 늘렸다.** 처음 판은 `SKIPPED · chosen 거짓`
단계 하나뿐이라 `Chosen` 을 통째로 버려도 안 죽었다. 같은 상태에 `chosen` 만
다른 단계를 하나 더 세워 **R76 과 R77 의 구별 자체**를 재게 했다.

```text
   커버리지   internal/panel   86.0% -> 88.5%   internal/runctl  97.1% -> 97.3%
             패키지별 하한 미달 0 · 전체 87.7%
   라우트     11 그대로 (R65 · CB0)
   node      99 중 99 초록 — 현황판을 안 건드렸다를 잰다
```

**선 위에서 화면을 한 번 돌렸다.** `page.go` 의 HTML 을 꺼내 실제 `card.mjs` 와
실제 파서가 낸 봉투로 올렸다 — 단계 다섯이 각자의 문장으로 섰고 (봉인된 것 ·
봉인 전 · 「경로가 갈려 안 갔다」 · 「골랐는데 못 닿았다」 · 실패 한 칸), 펼침이
`tool_use_id` 로 들고 단계끼리 안 섞였으며 원문 토글의 `pre` 에 자식 요소가
0 이었다. **이것은 게이트가 아니다** — 실제 하네스도 Mediator 도 안 탔고
그 둘이 CB2 다.

### 게이트 측정 (2026-09-17) — CB2 초록 · CB6 보류 해제

측정 나무 `measure/u6-cb2` (U6 위에 U7 을 얹었다 · `e3d987f`)를 지어 CT103 의
`enode-dev` 에 올리고 실 하네스로 쟀다. **Run 하나다** — `cb2-past-1`.
U8 은 넷이 필요했지만 CB2 는 「끝난 Run 을 눌러 본다」 하나이고, 같은 Run 을
도는 중에 한 번 더 누르면 「봉인 전」까지 함께 본다.

**모델을 계약이 haiku 로 못 박았다** (사용자의 사용량 사정). `agent.model` 이
`claude --model` 로 그대로 나가고, 시작 줄에 `claude-haiku-4-5-20251001` 로
찍혔다. **두 단계 합이 $0.0347 이다.** 모델 선택이 CB2 가 재는 것에 안 섞인다 —
파서도 렌더러도 출처도 모델 밖이다.

```text
   ①  같은 Run 을 두 번 눌렀다 — 봉인 전과 뒤

       봉인 전   source=progress · 걷힌 줄 **없다** · survey 15,343 바이트
                단계 2 는 CLAIMED 이고 사건 둘에서 자라는 중이었다
       봉인 뒤   source=sealed · 걷힌 줄 **있다** · survey 5,038 바이트
                「본문 17 개가 걷혔다 (11,533 바이트)」

       **같은 Run 이 15,343 에서 5,038 로 다른 물건이 됐다.**  두 신호가 같은
       답을 냈다 (R74) — progress 면 elided 가 없고, sealed 면 있다

   ②  출처가 GET 이고 tar 를 안 푼다

       제어판 -> Mediator 홉을 기록 프록시로 그대로 흘려 보며 쟀다.
       Run 하나를 누르면 나가는 것이 정확히 셋이다:

         GET /v1/runs/cb2-past-1
         GET /v1/runs/cb2-past-1/steps/1/log?as=raw&name=survey
         GET /v1/runs/cb2-past-1/steps/2/log?as=raw&name=note

       **record 호출 0 · log 호출 2 (= 단계 수) · 1+N 그대로.**
       name 에 Step.ID 가 실렸고 from 이 0 번이다

   ③  같은 파서의 같은 모양     단계마다 사건 19.  시작(모델·버전·도구 29) ·
                            생각 · 도구 Bash · 접힌 결과 · 말 ·
                            「끝 success · 턴 4 · $0.0181」
                            도는 것의 카드와 같은 함수가 그렸다 (R81)

   ④  verdict.checks          그대로 보인다.  이 유닛이 안 만졌다

   ⑤  펼침                    [펼치기] 를 누르면 본문이 선다.  열쇠가 tool_use_id
```

**CB2 초록이다.** 그리고 이것으로 **U8 의 CB6 보류가 풀린다** — 장면 ⑥ 의
나머지 절반(「제어판의 지난 작업에서 그 Run 을 누르면 같은 모양으로 읽힌다」)이
이 측정이다. 앞 절반(`runctl record` 의 tar 와 `GET` 이 같은 바이트)은 U8 의
측정에서 이미 초록이었다.

### 측정이 낸 관찰 하나 — 결함은 아니다

**봉인 뒤의 사건 열은 껍데기만 남는다.** `logShell` 의 허용목록이 본문을 어느
사건에서도 안 남기므로 (`transcript/shell.go`) 「말」과 「생각」과 「결과」가
줄은 있고 본문이 비어 있다. **그것을 설명하는 것이 걷힌 줄 하나다** —
「본문 17 개가 걷혔다 (11,533 바이트)」.

```text
   결함이 아닌 이유   봉인의 설계가 그렇다.  본문을 남기면 봉인된 Run 이
                   하네스 원문을 영구 보관하게 된다
   그래도 적는 이유   화면이 그 한 줄로 충분한가는 **사람이 고를 자리**다.
                   지금은 충분하다고 보고 넘어간다 — 걷힌 줄이 단계마다 있고
                   원문 토글이 봉인된 바이트 그대로를 보인다
```

### 측정 뒤 기계 상태

```text
   실행본     enode-dev 의 mediator 와 ~/.local/bin/enode 가 **측정 나무 것이다**
             백업 $HOME/.local/state/enode-dev/{mediator,enode}.bak-u6cb2-20260917-133503
   도는 것    mediator(8080) · 노드 exec · 제어판(8099).  전부 U6 코드다
   걷은 것    기록 프록시(8081)와 둘째 제어판(8100)과 nodes/exec-hop.yaml
   측정 나무   measure/u6-cb2 는 **병합하지 않는다.**  PR 은 unit/fleet-card
             다음에 unit/panel-past 다
```

### 모순 검사 — 막은 것 0 · 값을 정한 것 셋 · 넓힌 것 하나

**이번에는 막은 것이 없다.** 답 여섯이 서로를 안 무른다.

```text
   값 ①   봉투의 키 이름을 도는 것과 맞춘다.  꺼내는 식이 <봉투>.transcript.events
          로 같아야 화면 함수가 하나로 선다.  U5 가 그 이유를 미리 적어 뒀다

   값 ②   「한 번 읽고 끝」은 「누를 때마다 한 번」이다.  자동 갱신이 0 이라는 뜻이고
          영원히 한 번이 아니다.  같은 Run 을 봉인 전과 뒤에 누르면 다른 것이 온다 —
          그것을 설명하는 줄이 출처다 (NC-5)

   값 ③   걷힌 줄은 봉인된 것에만 있다.  **그래서 그것이 봉인 전과 뒤를 가르는
          둘째 신호다.**  출처 헤더가 첫째이고 둘이 같은 답을 내야 한다

   넓혔다  답 6 의 「남긴 것이 없다」 한 줄이 **셋**이 된다 — 「경로가 갈려 안 갔다」 ·
          「골랐는데 못 닿았다」 · 「돌았고 아무 말도 안 했다」.  Step.Chosen 이
          있는 이유가 정확히 앞의 둘을 가르려는 것이다
```

### 이 유닛이 치르는 값 하나

**봉인 전 Run 을 누르면 진행 파일 전체가 한 응답에 실린다.** 봉인 뒤는 선별본이라
작고(세 단계 합 16,445 바이트) 봉인 전은 크다(한 단계가 127,117). 브라우저 쪽은
루프백이라 안 아프고 **아픈 쪽은 Mediator 의 N+1** 이다. 누를 때 한 번이고
폴링이 아니다. NFR 단계가 없으므로(정본이 스킵으로 걸었다) 이 봉투를 설계에
적어 두고 **CB2 가 밟는다.**
...
### CB3 의 마지막 줄을 쟀다 — 초록 (2026-09-17)

**Mediator 를 10초 멈췄다 켰다.** Run `cb3-pause-2` (haiku · $0.0260) 가 도는
동안 mediator 프로세스를 죽이고 10초 뒤 다시 띄웠다.

```text
   실패가 났나     났다.  노드 로그 —
                 「cannot push the transcript chunk」 err=connection refused
                 그리고 「the transcript upload recovered」 failures=5 after=10s
   이어 붙었나     붙었다.  끊기 전 10,789 바이트 -> 뒤 15,348 바이트 (+4,559)
   두 벌이 생겼나   안 생겼다.  **뒤의 앞 10,789 바이트가 앞과 바이트까지 같다**
                 (cmp -n 10789 A B 가 조용하다)
   단계가 살았나    살았다.  reason=ok · 턴 6 · produced=[done.json]
```

**한 줄만 적히는 규율도 함께 섰다** — 실패 다섯에 로그 한 줄이다
(`upload.go` 의 「연속 실패의 첫 번째만 적는다」).

### 처음 쓴 주입은 실패를 안 만들었다 — 적어 둔다

`SIGSTOP` 으로 mediator 를 얼렸더니 **PUT 이 실패하지 않았다.** 커널이 연결을
받아 두고 클라이언트는 응답만 기다렸고, 주기 flush 가 `context.Background()` 를
쓰므로 제한시간이 없다. 노드 로그에 실패가 0 줄이었다 — 그때는 지연만 주입한
것이다. **프로세스를 죽여야 connection refused 가 난다.**
