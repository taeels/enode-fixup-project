...
| U8 | `fleet-card` | FR-7 | **CB4** · **CB6** | U4 · **U5 (코드)** · CB3 | W-e | **코드 섰음 · 게이트 미측정** |
...
```text
   CB4   보인다 (중앙)   목록 폴링을 막아도 카드가 자라는지.  원문 토글.  안 가리는지
   CB6   한 장면        scene-gates 1절 ① ~ ⑦
   CB1   재확인         렌더러를 옮긴 뒤 제어판의 카드 줄
```

**측정의 조건 하나** — `main` 의 Mediator 가 노드의 첫 시도(`attempt 0`) 청크를
400 으로 막는다. 그 한 줄 수정이 `unit/chunk-push` 에 있으므로 **CB4 를 눈으로
재려면 그 나무가 필요하다.** 이 유닛의 코드와 무관하고 진행자가 병합 순서로
푸는 자리다.
