...
**이 차이가 게이트 하나를 깼다** — CA5 의 계약은 오늘도 제출에서 거절된다.
`Validate` 에 `in.from` 의 정적 검사가 이미 있어서 (`contract.go:1109-1126` ·
`ADR-023` §6.2.1) 「어느 단계도 안 내는 이름」이 `400` 이다. 그 문구가 팩을
안 가리키는 것이 대가다 — `business-logic-model.md` 8절.
