...
```text
   ...
   R30   raw 는 한 줄이다 — Sub 와 바이트 수만.  본문 JSON 을 카드에 안 그린다.
         버리지도 않는다 (R22 그대로) — 줄 전체는 원문 토글이 낸다
```

**R30 은 CB1 이 낸 규칙이다.** 측정 전에는 raw 의 본문을 그대로 그렸고, 실 하네스로
재 보니 **도는 동안 사건 29 중 16 이 raw** 였다 — `system/thinking_tokens` 가 토큰
델타마다 한 줄씩 오고 `rate_limit_event` 가 그 사이에 낀다. 카드의 절반이 장부
줄이면 CB1 의 「읽을 수 있는 문장이 흐른다」가 서지 않는다.

**버리는 것과 다르다.** It's a Plan 의 같은 화면은 프로토콜(AG-UI) 어휘 밖의 사건을
`toRunEvent` 에서 **버린다** — 화면 어휘가 깨끗한 대신 하네스가 새로 내는 것을 영영
모른다. 이쪽은 접기만 한다. 파서의 「사건을 안 버린다」(requirements.md 5.3)가
그대로 서고, 원문 토글이 같은 응답의 바이트를 낸다.

**이 규칙의 검사는 CB1 이다** — 제어판의 JS 를 재는 자동 시험이 0 이기 때문이다
(계획 1.1). **U8 이 그 자리를 닫는다** — 카드 렌더러가 한 벌이 되면서 `.mjs`
하네스 안으로 들어온다.
