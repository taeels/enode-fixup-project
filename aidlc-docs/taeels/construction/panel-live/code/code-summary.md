...
## 8. CB1 뒤에 고친 것 하나 — raw 를 한 줄로 접는다 (R30)

**게이트를 재기 전에는 못 보는 것이었다.** 가짜 링 바이트로 돌린 시험은 raw 가
몇 줄이든 초록이고, 실 하네스에서야 **도는 동안 사건 29 중 16 이 raw** 인 것이
보였다 — `system/thinking_tokens` 가 토큰 델타마다 한 줄씩 온다.

```text
   전   evSummary 가 raw 의 e.text 를 그대로 냈다 — JSON 한 줄 통째가 카드에 들어간다
   후   Sub 와 바이트 수만 낸다 — raw · rate_limit_event · 418 바이트
        줄 전체는 원문 토글이 그대로 낸다 (R23 · 응답의 data 를 다시 안 읽는다)
```

**고친 자리는 `evSummary` 하나다.** `drawEvent` 도 봉투도 파서도 안 건드렸다 —
`internal/transcript` 의 diff 가 0 이고 사건은 그대로 다 온다.

```text
   다시 잰 것   같은 링(감긴 창 · 사건 39 · raw 9)을 새 빌드로 다시 그렸다.
               카드가 말 -> 도구 Bash -> raw 한 줄 -> 결과 Bash [펼치기] 로 읽힌다
               NC-2 · 원문 토글 · 끝 한 줄 전부 그대로
   안 돈 것     하네스를 다시 안 돌렸다.  링이 그 단계를 아직 들고 있어 필요가 없었고,
               wrap 단계가 계정의 다섯 시간 창을 태운 뒤라 다시 돌릴 자리가 아니다
   자동 시험    **0 이다.**  제어판의 JS 를 재는 하네스가 이 저장소에 없다 (계획 1.1).
               R30 의 검사는 CB1 이고, U8 이 렌더러를 한 벌로 만들며 그 자리를 닫는다
```

**It's a Plan 과 갈리는 자리다.** 그쪽은 AG-UI 어휘 밖의 사건을 `toRunEvent` 에서
버린다 — 화면은 깨끗하지만 하네스가 새로 내는 것을 영영 모른다. 이쪽은 접기만
하고 원문에 그대로 남긴다.
