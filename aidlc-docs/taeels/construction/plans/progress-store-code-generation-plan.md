...
- [ ] 2.3 이름은 `NN-<safe(name)>.<attempt>.log` (R2). `%02d` 와 `%d` 는
      `blobPath` (`record.go:270`) 의 규칙을 따른다
- [ ] 2.4 `parseProgressName(n string) (seq, attempt int, name string, ok bool)` —
      오른쪽에서 읽는다 (R6). `.log` 를 떼고, 마지막 `.` 뒤를 `attempt` 로,
      첫 `-` 앞을 `seq` 로 읽는다. 못 읽으면 `ok=false` 이고 부르는 쪽이
      조용히 건너뛴다 (R34)
- [ ] 2.5 주석에 **`parseBlobName` 을 재사용 못 하는 이유**를 적는다 —
      그쪽은 `%02d.%d-이름` 이라 왼쪽에서 읽는다 (`record.go:304`)
- [ ] 2.6 권한 — 디렉터리 `0700` · 파일 `0600` (R4). 주석에 `logs/` 와 다른
      이유를 적는다 (`record.go:47` 의 `0755` · `record.go:59` 의 `0644`) —
      그쪽은 선별본이고 이쪽은 원문이다

### Step 3 `[A]` — 단계별 뮤텍스

- [ ] 3.1 `Store` 에 필드 하나를 더한다. 오늘 `Store` 는 `struct{ Root string }`
...
- [ ] 3.3 `acquire(runID)` 가 `locks.mu` 안에서 `refs++` 하고,
      ...
      호출은 같은 객체를 받는다
- [ ] 3.4 주석에 그 함정을 문장으로 적는다 — 「뮤텍스를 맵에서 지우는 순간
      그것을 쥔 쓰기가 있으면 다음 호출이 새 뮤텍스를 만들어 둘이 같은 파일에서
      동시에 돈다」
- [ ] 3.5 선례를 가리킨다 — `Ring` 이 같은 자리를 뮤텍스 하나로 풀었다
...
- [ ] 4.1 ① 키를 잠근다. ② ~ ⑦ 이 전부 이 안이다 (R28 · 1.4 의 셋)
- [ ] 4.2 ② `MkdirAll(progressDir, 0700)`. 게으르다 — `Store.Open`
      (`record.go:45`) 을 안 건드린다 (R3). 주석에 이유를 적는다: `Seal` 이
      `s.Open` 을 부르므로 (`record.go:96`) `Open` 에 넣으면 지우기 직전에
      다시 만든다
- [ ] 4.3 ③ 디렉터리를 훑어 그 `(seq, name)` 의 최대 시도를 읽는다 (R5 · R6).
      곁파일이 0 이라 이름이 진실이다
- [ ] 4.4 ④ 세 갈래 —
      `attempt < 지금` 버린다 (0 바이트 · 지금의 `Progress`) R8 ·
      `attempt == 지금` 이어 붙인다 R7 ·
      `attempt > 지금` 앞 시도 파일을 전부 걷고 새 이름으로 연다 R9 · R16
- [ ] 4.5 ④ 의 셋째에서 **걷기가 실패해도 진행을 안 막는다** (R25). 걷는 범위는
      같은 `(seq, name)` 의 더 작은 `attempt` 뿐이다 — 다른 단계도 다른 Run 도
      안 건드린다
- [ ] 4.6 ⑤ 이미 닿았으면 0 바이트를 쓰고 ⑦ 로 (R15). **판정의 근거는 표시
      줄의 존재다. 크기가 아니다** (R14)
- [ ] 4.7 ⑥ 두 갈래 — Step 5
- [ ] 4.8 ⑦ `Total` 은 쓰기 직후의 파일 크기다. 락 안에서 잰다 (R13 · 7.2)
...
- [ ] 5.6 `Bytes` 가 `int64` 인 것이 `elidedMark` 의 `int` 와 다른 유일한 자리다.
      근거를 주석에 적는다 — `limit` 이 `int64` 다 (`config.go:51` 의
      `MaxBlobBytes int64`)
- [ ] 5.7 `Capped` 를 마지막 줄로 판정한다 (R14). 꼬리 한 번 읽기면 족하다 —
      R15 가 「표시 줄이 언제나 마지막」을 보장한다
- [ ] 5.8 **`Total > limit` 이 정상이다** (R45). 주석이 그것을 적고 Step 13 의
      시험이 못 박는다
...
- [ ] 6.1 ① 키를 잠근다 (쓰기와 같은 키) · ② 최대 시도를 고른다 (R5).
      **`attempt` 인자가 없다** — 그것이 R17 을 이 유닛 안에서 참으로 만든다
- [ ] 6.2 ③ `Capped` 를 마지막 줄로 판정한다 (R14)
- [ ] 6.3 ④ 세 갈래 — 파일이 없으면 `Total 0 · Attempt 0 · Capped false` 에
      빈 본문이고 404 가 아니다 (R18) · `from >= Total` 이면 빈 본문 ·
      `from < Total` 이면 `from` 부터
- [ ] 6.4 **`from > Total` 도 오류가 아니다** (R19). 총 길이가 뒤로 간 뒤의
      폴링이 그 모양이다
- [ ] 6.5 본문을 **`Total - from` 에서 자른다** (5절 ③). `io.LimitReader` 로
      ...
      「두 벌이 안 생긴다」다
- [ ] 6.6 본문은 락 밖에서 읽힌다. 그 사이에 `DropProgress` 가 돌아도
      리눅스에서 열린 fd 는 살아 있으므로 읽기가 안 깨진다 — 주석에 적는다

### Step 7 `[A]` — `DropProgress`

- [ ] 7.1 ① 그 Run 의 `drop` 을 Lock 한다 (R31) · ② 진행 트리를 통째로
      지운다 (`os.RemoveAll`) · ③ 뮤텍스 맵에서 그 Run 을 걷는다 (R30)
- [ ] 7.2 멱등이다 (R21). 없으면 아무것도 안 하고 `nil` 을 낸다.
      `os.RemoveAll` 이 이미 그 성질이다
- [ ] 7.3 **Run 단위다. 단계 단위가 아니다** — 봉인도 종료도 쓸기도 Run 단위로
      일어난다

### Step 8 `[A]` — `SweepProgress` — 고아 쓸기

- [ ] 8.1 **디렉터리를 먼저 읽는다.** `<Root>/progress/` 가 없거나 비었으면
      **0 을 내고 끝낸다** — 부르는 쪽이 DB 를 안 치게 한다 (5절 ④)
- [ ] 8.2 `live` 를 `safe()` 로 접어 집합을 만든다. **앞으로 가는 사상만
      쓴다** — `safe()` 는 되돌릴 수 없으므로 디렉터리 이름에서 `runID` 를
      복원하지 않는다 (5절 ⑤)
- [ ] 8.3 **조건 둘은 배타이고 순서가 규칙이다** (R37) — **Run 상태를 먼저
      보고 종료가 아닐 때만 나이를 본다**. 뒤집으면 종료된 Run 의 트리가 6시간
      남아 V4 = 0 과 어긋난다
- [ ] 8.4 조건 ① — 살아 있는 집합에 없는 디렉터리는 통째로 지운다.
      유예 0 (V4). **DB 에 없는 Run 도 여기로 온다** (R38)
- [ ] 8.5 조건 ② — 살아 있는 Run 의 디렉터리에서 파일마다 mtime 을 보고
      `now.Sub(mtime) > maxAge` 면 그 파일만 지운다 (V5 · 3.2).
      같은 Run 의 방금 쓴 다른 단계 파일은 안 건드린다
- [ ] 8.6 조건 ② 는 그 파일의 `(seq, name)` 뮤텍스를 잡고 지운다.
      **이름이 안 읽히면 건너뛴다** (R34) — 키를 못 만들기 때문이고, 그 Run 이
      끝나면 조건 ① 이 통째로 걷는다
- [ ] 8.7 조건 ① 은 그 Run 의 `drop` 을 Lock 하고 지운다 — `DropProgress` 와
      같은 경로를 쓴다
- [ ] 8.8 지운 수를 돌려준다. 실패는 보조다 (R25) — 한 항목이 실패해도
      나머지를 계속 돈다
- [ ] 8.9 **`Record` 를 안 건드린다** (R39). 주석에 적는다 — 구멍 ① ② 가 낳는
      「종료했는데 안 봉인된 기록」은 이 유닛의 것이 아니다

### Step 9 `[A]` — `internal/record/record.go` — 두 자리

- [ ] 9.1 **`Seal` 의 첫 줄** (R22 · R47) — `if s.Sealed(runID) { return nil }`
      **보다 앞이다**
...
- [ ] 9.2 주석에 **막아야 할 것이 `seal(d)` 의 chmod** 임을 적는다 —
...
- [ ] 10.1 `sealRecord` 의 **`s.Records == nil` 검사 바로 뒤**에
...
- [ ] 10.2 실패는 삼키고 로그로 낸다 (R25 · R48). `s.log()`
      (`store.go:65`) 를 쓴다. 메시지는 영어다 (`CONVENTIONS.md` 2.1)
- [ ] 10.3 주석에 **`Seal` 의 성공과 무관하게** 부르는 이유를 적는다

### Step 11 `[B]` — `internal/store/reap.go`

- [ ] 11.1 쓸기를 **`Reap()` 안**에 둔다. **`sealExpired` 안이 아니다** (V2) —
      `sealExpired` 는 `if n > 0` 뒤라 (`reap.go:99`) 그 주기에 회수가 0 이면
      아예 안 훑는다
- [ ] 11.2 자리는 `if n > 0 { ... }` 블록 뒤, `return n, nil` 앞이다.
      **조건 없이 매 주기 돈다**
- [ ] 11.3 살아 있는 Run 을 낸다 —
      `SELECT run_id FROM runs WHERE state NOT IN ('SUCCEEDED','FAILED')`
- [ ] 11.4 `s.Records.SweepProgress(live, record.ProgressMaxAge, time.Now())`
- [ ] 11.5 **실패가 `Reap` 을 안 죽인다** (R25). 로그만 내고 `n` 을 안 바꾼다
- [ ] 11.6 **새 타이머 0 · 새 고루틴 0 · 새 설정 키 0** (V1). 이미 도는
      ...
      **기동에 한 번 먼저 돌게** 한다. R24 의 「기동과 주기마다」가 글자 그대로
      이미 거기 있다
- [ ] 11.7 `s.Records == nil` 이면 건너뛴다 — `reap.go:114` 의 결과 같다

### Step 12 `[A]` — 시험 — 자리 · 이름 · 시도 · 뮤텍스

새 파일 `internal/record/progress_test.go`.

- [ ] 12.1 R1 — 진행 파일이 `<Root>/progress/run-<id>/` 에 나고
      `<Root>/run-<id>/` 안에는 0 개다
- [ ] 12.2 R2 · R6 — `01-build.step.2.log` 를 넣으면 단계 이름 `build.step` ·
      시도 `2` 가 나온다. **왼쪽 파싱이면 이 시험이 빨개진다**
- [ ] 12.3 R4 — 파일 `0600` · 디렉터리 `0700` 을 `Stat` 으로 잰다
- [ ] 12.4 R3 — `Store.Open` 만 부르면 `<Root>/progress/` 가 안 생긴다
- [ ] 12.5 R7 · R8 — 같은 시도는 이어 붙고, 작은 시도는 **0 바이트를 쓰고
      지금의 `Progress`** 를 낸다
- [ ] 12.6 R9 · R16 · R17 — 시도 1 을 쓰고 시도 2 를 쓰면 `ReadProgress` 가
      시도 2 의 바이트만 내고 시도 1 의 파일이 디스크에 없다
- [ ] 12.7 R18 — 파일이 없는 단계는 `Total 0 · Attempt 0 · Capped false` 에
      빈 본문이다. 오류가 아니다
- [ ] 12.8 R19 · R20 — 경로 둘의 표시가 다르다. 시도가 바뀌면 `Attempt` 가
      바뀌고, 트리가 걷히면 `Total` 과 `Attempt` 가 함께 0 이다
- [ ] 12.9 R21 — `DropProgress` 를 두 번 불러도 오류가 0 이다
- [ ] 12.10 R29 — **키가 다르면 안 기다린다.** 단계 둘에 동시에 쓰고 둘 다
      끝나는지 잰다
- [ ] 12.11 R28 · R30 · R31 — 고루틴 여럿이 같은 키에 쓰는 동안 `DropProgress`
      를 섞는다. **`go test -race` 로 잰다** (6절 3번)
- [ ] 12.12 R32 — `TestPathTraversalIsBlocked` (`record_test.go:300`) 의 결을
      진행 트리에도 건다. `../../etc/passwd` 가 `____etc_passwd` 로 접힌다
- [ ] 12.13 R6 · R34 — 못 읽는 이름을 디렉터리에 두고 오류가 0 인지 잰다

### Step 13 `[A]` — 시험 — 상한 · 개행 보장 · 틈 넷

- [ ] 13.1 R10 — 상한 안의 청크가 통째로 들어간다 (개행에 안 맞춘다)
- [ ] 13.2 R11 — 상한을 넘기면 마지막 개행까지만 남고 반쪽 줄이 0 이다
- [ ] 13.3 **R14 의 핵심 시험** — 개행이 없는 큰 청크 하나로 상한을 넘기면
      `Total < 상한` 인데 `Capped` 가 참이다. **크기 비교로 적은 구현은
      여기서 빨개진다**
- [ ] 13.4 R12 · R15 — 표시 줄이 정확히 한 번이고 언제나 마지막 줄이다.
      닿은 뒤에 또 PUT 이 와도 안 자란다
- [ ] 13.5 **R43 의 시험** — 꼬리가 반쪽 줄인 파일에 상한을 넘기면, 표시 줄
      앞에 개행이 하나 들어가 마지막 줄이 온전한 표시 줄이 된다.
      **개행 보장을 걷으면 이 시험이 빨개진다**
- [ ] 13.6 틈 넷 (`nfr-design-patterns.md` 1.3) — 네 자리에서 멈춘 파일을
      손으로 지어 놓고 다음 호출이 **전부 「표시 줄이 온전히 마지막 줄」로
      수렴**하는지 잰다. 특히 셋째 (표시 줄이 반쪽으로 남은 파일) 에서 R43 이
      그 조각을 개행으로 닫는지
- [ ] 13.7 R45 — `Total > limit` 을 **정상으로 못 박는 시험**을 둔다.
      `Total <= limit` 을 불변식으로 적지 않는다. 주석이 그 이유를 적는다 —
      **이 시험이 없으면 다음 사람이 `assert Total <= limit` 을 쓴다**
- [ ] 13.8 R13 — `Total` 이 언제나 `os.Stat` 의 크기와 같다. 표시 줄과 R43 의
      개행을 포함해서
- [ ] 13.9 R27 — `AppendLog` 을 상한의 절반씩 두 번 불러 **총 길이가 상한과
      같아져도** 잘림 표시가 **안 붙는지** 잰다. 총 길이로 판정한 구현은 여기서
      빨개진다. 그리고 반환값이 총 길이인지 잰다 (D4)

### Step 14 `[A]` — 시험 — 봉인 경로 불변식

- [ ] 14.1 R35 — 진행 파일을 쓴 Run 을 봉인하고 `Tar` 를 풀어
      `progress/` 항목이 0 개인지 잰다. `TestTarIsSelfSufficient`
      (`record_test.go:109`) 의 결을 따른다
- [ ] 14.2 R36 — 봉인 뒤에도 진행 트리를 지울 수 있는지 잰다
      (쓰기 비트가 살아 있다). `newStore` 의 `unseal` 정리
      (`record_test.go:17` ~ `:22`) 를 그대로 쓴다
- [ ] 14.3 R22 · R47 — 봉인이 진행 트리를 지운다. 그리고 **이미 봉인된
      Run 에 다시 `Seal` 을 불러도** 늦은 청크가 만든 트리를 지운다
      (조기 반환 뒤에 두면 이 시험이 빨개진다)
- [ ] 14.4 R48 — `DropProgress` 가 실패하는 상황에서도 봉인이 선다.
      진행 트리 자리에 지울 수 없는 것을 놓아 실패를 만들고 `Seal` 이
      `nil` 을 내는지 잰다
- [ ] 14.5 `TestSealIsIdempotent` (`record_test.go:62`) 가 그대로 초록인지

### Step 15 `[B]` — 시험 — 쓸기 (DB 를 탄다)

새 파일 `internal/store/progress_test.go`.

- [ ] 15.1 V4 · R37 — 종료 상태 Run 의 진행 트리가 `Reap()` 한 번에 사라진다
- [ ] 15.2 R38 — DB 에 없는 Run 의 트리가 걷힌다
- [ ] 15.3 V5 · 3.2 — 살아 있는 Run 의 6시간 넘은 파일이 걷히고
      **같은 Run 의 방금 쓴 파일은 안 걷힌다**. `now` 인자를 앞으로 당겨 잰다 —
      슬립이 0 이다
- [ ] 15.4 R37 의 순서 — 종료 Run 의 방금 쓴 파일도 걷힌다 (나이를 먼저
      보는 구현은 여기서 빨개진다)
- [ ] 15.5 R39 · R35 — 쓸기 뒤에도 `Record` 가 그대로다. `Tar` 출력에
      `progress/` 가 0 개다
- [ ] 15.6 V2 — 쓸기가 **회수가 0 인 주기에도** 돈다 (`sealExpired` 안이면
      이 시험이 빨개진다)
- [ ] 15.7 R23 — `StepFiles` 가 실패해 `Seal` 이 안 불리는 경로에서도
      트리가 걷힌다
- [ ] 15.8 8.1 — `<Root>/progress/` 가 없으면 DB 질의가 0 번 돈다
...
- [ ] 16.1 박는 자리 — `internal/record/progress.go` 의 `cappedMark`
      구조체 하나와 마샬 함수 하나 (Step 5.5). 이 저장소에서 그 글자가 나는
      자리는 여기 하나다
- [ ] 16.2 못 박는 바이트 — 실측이다 (2026-09-15)
...
- [ ] 17.1 ① R43 의 개행 보장을 걷는다
- [ ] 17.2 ② R14 를 크기 비교(`Total >= limit`)로 바꾼다
- [ ] 17.3 ③ R6 의 오른쪽 파싱을 왼쪽으로 바꾼다
- [ ] 17.4 ④ R9 의 앞 시도 걷기를 걷는다
- [ ] 17.5 ⑤ R22 의 `DropProgress` 를 조기 반환 뒤로 옮긴다
- [ ] 17.6 ⑥ R27 의 잘림 판정을 총 길이로 바꾼다
- [ ] 17.7 ⑦ R37 의 순서를 뒤집는다 (나이를 먼저 본다)
- [ ] 17.8 ⑧ R1 의 자리를 `<Root>/run-<id>/progress/` 로 옮긴다 —
...
- [ ] 18.1 6절의 1 ~ 10 을 순서대로
- [ ] 18.2 안 초록인 것이 있으면 **그대로 적는다.** 초록이라고 안 적는다

### Step 19 `[—]` — 문서

- [ ] 19.1 `construction/progress-store/code/code-summary.md` —
      만든 파일 · 고친 파일 · 잰 값 · 16.2 의 글자 · 못 닫은 것
- [ ] 19.2 이 계획의 체크박스를 전부 `[x]` 로
- [ ] 19.3 `aidlc-docs/taeels/aidlc-state.md` 의 U3 절과 `audit.md` —
      **Part 2 에서** 쓴다. 이 단계(Part 1)의 커밋에는 안 싣는다
- [ ] 19.4 회차 문서와 팩은 **안 고친다.** 9절이 진행자에게 넘길 글자만 적는다
...
```text
   ...
   나온 것    **21 자리 · 파일 열**.  그중 둘이 internal/record/record_test.go 다 —
             **U3 이 만지는 파일이다**
   ...
   U3 이 안 하는 것  기존 21 자리를 고치는 것.  열 파일 중 여덟이 이 유닛의
                    파일 행렬 밖이고, 고치면 게이트 2 의 diff 0 이 깨진다
   어디로 가나       **진행자다** (9절)
```
...
9.1 과 별개의 자리다. **6.3 의 실측이 근거다** — 장식 문자 21 자리가 `main` 에
있고 `glyphscan.go` 가 그 절반(ASCII 두 별표 · 주석)을 안 본다. **U3 은 새로
안 만들고 기존 것을 안 고친다** (파일 행렬 밖이고 게이트 2 의 diff 0 이 깨진다).
