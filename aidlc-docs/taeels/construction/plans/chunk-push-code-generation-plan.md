...
### Step 1 — 겉면을 못 박는다
- [ ] `LogChunkAck` · `PutLogChunk` 시그니처 (몸통은 Step 3)
- [ ] `uploader` 와 `newUploader` · `Write` · `Close` 시그니처
- [ ] `transcript(step)` 의 새 시그니처
- [ ] `go build ./...` 가 선다

### Step 2 — `api.go` 의 `attempt` 검증 (R6) — **착수 조건이다**
- [ ] `attempt <= 0` 을 `attempt < 0` 으로. 음수는 여전히 400
- [ ] 주석에 왜인지 적는다 — `seq` 는 1 부터이고 `attempt` 는 0 부터다
- [ ] 시험: `attempt=0` 이 200 이고 진행 파일이 생긴다 (**저장소에 0 이던 줄**)
- [ ] 시험: `attempt=-1` 은 그대로 400
- [ ] 기존 진행 청크 시험 다섯이 그대로 초록

### Step 3 — `Client.PutLogChunk` (R5 · R10)
- [ ] `PUT .../log?name=<n>&progress=1&attempt=<a>` · `text/plain`
- [ ] 응답 헤더 셋을 다 읽어 `LogChunkAck` 로 낸다
- [ ] 2xx 가 아니면 오류. 몸통의 `reason` 을 싣는다
- [ ] `UploadLog` 를 **한 글자도 안 고친다** (R23)

### Step 4 — `uploader` 의 `Write` (R1 · R2 · R3)
- [ ] 버퍼에 넣고 **바로 돌아온다.** 네트워크를 0 번 기다린다
- [ ] 언제나 `len(p), nil`. 오류를 0 번 낸다
- [ ] 1 MiB 를 넘으면 `done` 을 세우고 더 안 받는다. **그래도 바로 돌아온다**
- [ ] 64 KiB 가 쌓이면 보내는 고루틴을 깨운다 (막히지 않게 — non-blocking)

### Step 5 — 보내는 고루틴 (R9 · R11)
- [ ] 2초 주기. 즉시 문턱으로도 깨어난다
- [ ] 버퍼를 통째로 떼어 온다. 비었으면 다음 주기로
- [ ] 실패하면 **뗀 것을 버퍼 앞에 도로 붙인다** (R11)
- [ ] 오프셋은 **이 고루틴의 지역 변수**다. 구조체 필드로 안 둔다 (R9)
- [ ] `recover` 로 패닉을 잡는다. 노드를 안 죽인다 (R4)

### Step 6 — 응답 셋을 읽는 순서 (R7 · R8 · R12)
- [ ] `Attempt` 를 **`Total` 보다 먼저** 본다 (R8)
- [ ] `Attempt` 가 내 것보다 크면 **멈춘다.** 오프셋을 안 옮긴다 (R7)
- [ ] `Capped` 면 `done` 을 세우고 멈춘다. **실행은 계속된다** (R12)
- [ ] 그 밖이면 오프셋을 `Total` 로 맞춘다 (R10)

### Step 7 — 실패 로그의 규율 (R 5절 · 답 8 = A)
- [ ] 연속 실패의 **첫 번째만** 적는다
- [ ] 회복한 첫 번째를 적는다 — 몇 번 실패하고 몇 초 만인지
- [ ] 버퍼가 넘쳐 멈춘 것을 한 줄로 적는다 (R14 — 중앙은 이것을 못 안다)
- [ ] 상한에 닿아 멈춘 것을 한 줄로 적는다 (R12)

### Step 8 — `transcript(step)` 와 배선 둘 (R18)
- [ ] 링이 nil 이어도 업로더는 돈다. 업로더가 nil 이어도 링은 돈다
- [ ] 에이전트 단계 (`claim.go:794`) 에 잇는다
- [ ] 명령 단계 (`claim.go:632`) 에 잇는다 — **규칙이 하나다**
- [ ] 정리를 `defer` 로 든다. **`UploadLog` 보다 먼저 비운다** (R17)

### Step 9 — 시험: 올라간다
- [ ] 도는 동안 청크가 실제로 PUT 된다. 쿼리 셋이 맞다
- [ ] 두 번 부르면 서버의 총 길이가 **늘어 있다** (CB3 의 첫 줄이다)
- [ ] 명령 단계도 올라간다 (R18)
- [ ] 단계 끝에 꼬리가 비워진 뒤 `UploadLog` 가 간다 (R17 — 순서를 잰다)

### Step 10 — 시험: 안 막는다 · 안 잃는다
- [ ] 서버를 죽여 놓고 `Write` 의 시간을 잰다 — **막히지 않는다** (R2)
- [ ] 서버를 10초 죽였다 살리면 **다음 청크가 이어 붙고 두 벌이 안 생긴다** (CB3)
- [ ] 버퍼가 넘치면 멈추고 `Write` 는 여전히 바로 돌아온다 (R3 · R14)
- [ ] 넘친 뒤 서버가 살아나도 **재개하지 않는다** (R15)

### Step 11 — 시험: 시도와 상한
- [ ] `attempt 0` 으로 보낸다 (R5) — `+1` 을 안 한다
- [ ] 응답의 `Attempt` 가 크면 멈춘다 (R7)
- [ ] `Capped` 면 멈추고 **실행은 계속된다** (R12)
- [ ] 상한 뒤로 왕복이 0 이다 (전역 한도를 안 갉는다)

### Step 12 — 변이와 게이트
- [ ] 변이 일곱 (5절)
- [ ] `eval "$(scripts/testdb.sh)"` 뒤 CP0 계열 전부
- [ ] 라우트 18 그대로 · `diff 0` 이어야 할 곳 전부 0
...
```text
   ①  attempt 검증을 <= 0 으로 되돌린다        Step 2 의 attempt=0 줄
   ②  노드가 attempt+1 로 보낸다              Step 11 의 첫 줄
   ③  Write 에서 동기로 보낸다                 Step 10 의 첫 줄 (**멈춤으로 빨갈 것이다**)
   ④  실패한 청크를 도로 안 붙인다              Step 10 의 10초 줄 (바이트가 샌다)
   ⑤  Total 을 Attempt 보다 먼저 본다          Step 11 의 셋째 줄
   ⑥  버퍼 상한을 뺀다                        Step 10 의 넘침 줄 (**메모리가 는다**)
   ⑦  Close 를 UploadLog 뒤로                 Step 9 의 순서 줄
```
