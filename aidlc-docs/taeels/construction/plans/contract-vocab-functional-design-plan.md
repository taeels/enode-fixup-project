...
`fetch.pack` 은 이 Run 에 없고, `claim.go:505-535` 가 **없는 입력을 값으로**
취급하므로 단계는 죽지 않고 `$IN` 에 팩이 없는 채로 돈다. CA5 를 그대로 돌리면
빨간 이유가 팩 코드가 아니라 명령의 이름 불일치가 된다. **그 게이트는 U5 의
것이지만 어휘의 주인은 이 유닛이다** — 질문 6 이 어디에 적을지 정한다.
