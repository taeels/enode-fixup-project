...
가짜 DOM 이 못 재는 것을 CB4 가 실제로 쟀다. 사각형을 재서 **카드가 그래프와
목록 어느 쪽과도 겹치지 않는 것**을 확인했고(겹침 0), 스크롤 따라가기와
접기/펼치기가 실 브라우저에서 도는 것을 봤다. 남은 것은 CSS 의 모양뿐이고
그것은 `.pen` 이 정한다.

**그리고 시험이 못 잡은 것을 하나 냈다** — 8절 ③ 의 자리가 아니라 새 자리다.
`GET log?as=events` 가 빈 로그에 `{"events":null}` 을 냈다. 시험 백열하나가
전부 초록인 채였고, 실 하네스의 가장 흔한 상태(아직 아무 말도 안 한 단계)에서만
났다. **계약 검사(R55)가 그것을 오류로 세웠기 때문에 보였다** — 안 세웠으면
빈 카드로 조용히 지나갔고, 그것이 R55 를 넣은 이유였다.

```text
   고친 자리   internal/transcript 의 Result.MarshalJSON.  **회차 밖이다** —
              U1 의 패키지이고 U8 의 행렬에서 diff 0 이라 별도 브랜치에서 고쳤다
              (fix/transcript-events-is-always-an-array)
   같은 자리   PR #45 의 store.Verdict.Checks.  타입이 자기 선 위 모양을 진다
```
...
```text
   CB4   **초록**   여섯 줄 전부.  목록 폴링을 막아도 카드가 자란다 (거절 12건 동안
                   로그 호출 8건).  40,969 -> 99,997 바이트 / 15초
   CB1   **초록**   제어판이 옮긴 렌더러 위에서 그대로 선다.  상태 줄이 하나 늘었다
   재확인
   CB6   **보류**   ① ~ ⑤ · ⑦ 초록.  ⑥ 의 record 대 GET 도 같은 바이트.
                   제어판의 지난 작업만 아직 JSON 원문이다 — **U6 을 안 지었다**
```

**보류는 통과가 아니다.** 빨간 이유가 U8 밖(U6 미착수)이라
`scene-gates.md` §4 대로 적고 넘어간다. 자세한 것은 `aidlc-state.md` 의
「게이트 측정」 절이다.

## 7.1 측정의 조건이었던 것 — 실제로 그랬다

`main` 의 Mediator 는 노드의 첫 시도(`attempt 0`) 청크를 400 으로 막아 실시간
경로가 죽는다. 그 한 줄 수정이 `unit/chunk-push` 에 있어서 U7 과 U8 이 한 나무에
있어야 했고, 측정 나무 `measure/u8-cb4` 를 따로 지어 잰 이유가 그것이다.

**U8 의 브랜치에 U7 을 안 합쳤다** — 합치면 U8 의 PR 이 U7 의 커밋을 싣는다.
측정 나무는 버린다.
