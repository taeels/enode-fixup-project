...
func run() int {
	...
	client := &enode.Client{
		...
		Poll: &http.Client{},
		// 업로드도 요청마다의 제한을 두지 않는다 (FR-3). 마감은 업로드 예산의 ctx 가
		// 정한다 — 30초 client 로 올리면 예산 3분 안에서도 큰 산출물 하나가 30초에
		// 끊긴다. 롱폴 client 와 모양은 같지만 칸을 나눈다: 쓰임이 다르고, 한쪽을
		// 조이면 다른 쪽이 따라 조여지면 안 된다.
		Upload: &http.Client{},
	}
